...
export const openView = {
  ...
  async getBoardMainData (param, nonLoadingYn) {
    ...
    var result = await commonAxiosFunction({ url: '/sUniB/tpgetCabinetMainBoard', param: Object.fromEntries(paramMap) }, nonLoadingYn)
  ...
  async getContentDetailData (inputParam, nonLoadingYn) {
    ...
    var contentDetail = await commonAxiosFunction({ url: '/sUniB/tpgetMyContentsList', param: paramSet }, nonLoadingYn)
  ...
  async getMainBoard () {
    ...
    var response = await commonAxiosFunction({ url: '/sUniB/tpgetMainBoard', param: Object.fromEntries(paramMap) }, false)
  ...
  async getUnknownMainBoard () {
    var paramMap = new Map()
    var response = await axios.post('/sUniB/tpgetUnknownMainBoard', Object.fromEntries(paramMap)
    )
    if (response.status === 200 || response.status === '200') {
      // eslint-disable-next-line no-new-object
      var resultObject = new Object()
  ...
  async getSearchMainBoard (nonLoadingYn) {
    ...
    var response = await commonAxiosFunction({ url: '/sUniB/tpgetSearchMainBoard', param: Object.fromEntries(paramMap) }, nonLoadingYn)
    // eslint-disable-next-line no-debugger
    debugger
  ...
  async getMainChanList () {
    ...
    // var response = await commonAxiosFunction({ url: '/sUniB/tpgetSearchMainBoard', param: Object.fromEntries(paramMap) }, false)
    // eslint-disable-next-line no-debugger
    debugger
    console.log(response)
  ...
  async getWriteBoardData (teamKey) {
    ...
    var response = await commonAxiosFunction({ url: '/sUniB/tpgetCabinetListForMyShareType', param: Object.fromEntries(paramMap) }, false)
    console.log(response)
    ...
    // paramMap.set('userKey', store.getters['D_USER/GE_USER'].userKey)
    // // console.log(paramMap)
    // var response = await commonAxiosFunction({ url: '/sUniB/tpgetTeamMenuList', param: Object.fromEntries(paramMap) }, false)
    // console.log(response)
    // if (response.status === 200 || response.statusText === 'OK') {
  ...
  async getOpenChannelListPopData (tabName) {
    ...
    var result = await commonAxiosFunction({ url: '/sUniB/tpgetUserTeamList', param: Object.fromEntries(paramMap) }, false)
    console.log(result)
  ...
  async getOpenPushListPopData (inputParam) {
    ...
    var result = await commonAxiosFunction({ url: '/sUniB/tpgetMyContentsList', param: paramSet }, false)
    console.log(result)
    return result.data
  }
}
