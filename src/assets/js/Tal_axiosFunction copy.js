...
export async function getTeamList (paramMap) {
  var resultList = null
  await this.$axios.post('https://mo.d-alim.com:9443/service/tp.getUserTeamList', Object.fromEntries(paramMap)
  ).then(response => {
...
export async function getContentsList (inputMap) {
  ...
  var result = null
  await this.$axios.post('https://mo.d-alim.com:9443/service/tp.getContentsList', Object.fromEntries(paramMap)
  ).then(response => {
...
export async function getContentsList (inputParam) {
  ...
  var result = null
  await this.$axios.post('https://mo.d-alim.com:9443/service/tp.getContentsList', param
  ).then(response => {
    result = response.data
  }).catch((error) => {
    result = error
...
export async function saveUser (userProfile) {
  ...
  param.user = user
  await axios.post('https://mo.d-alim.com:9443/service/tp.saveUser', param
  ).then(response => {
    if (response.data === 'OK') {
...
export async function saveUserDo (inputParam) {
  ...
  param.ownUserKey = JSON.parse(localStorage.getItem('sessionUser')).userKey
  var result = null
  await this.$axios.post('https://mo.d-alim.com:9443/service/tp.saveUserDo', param
  ).then(response => {
    result = response.data
  }).catch((error) => {
    result = error
