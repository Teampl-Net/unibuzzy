...
export const commonMethods = {
      ...
      if (this.$dayjs(compareDate).format('MM') === this.$dayjs(now).format('MM')) {
        if (this.$dayjs(compareDate).format('DD') === this.$dayjs(now).format('DD')) {
          ...
          if (distance > 0) {
            ...
            innerHTML += '<p class="commonColor font14 fl" style="text-decoration: underline;"> 취소 </p>'
            innerHTML += '<img src="/resource/common/icon_alarm_red.svg" class="fl img-w13 mleft-05 mright-03">'
            innerHTML += text
            innerHTML += '</div>'
            text = innerHTML
          } else {
  ...
  async makeShareLink (key, type, message, title, jobkindId) {
    var paramMap = new Map()
    paramMap.set('pageType', type)
    paramMap.set('targetKey', key)
    if (jobkindId) {
      paramMap.set('jobkindId', jobkindId)
    }
    if (message) {
  ...
  findUrlChangeAtag (inputText) {
    console.log('findUrlChangeAtag', inputText)
