<template>
  ...
  <!-- <div class="w-100P" style=" height:100vh;top: 50px; position: absolute; overflow:auto"> -->
    ...
    <div style="min-height: 400px; height: 100%;">
      ...
      <gConfirmPop :confirmText="modiYn? this.$t('COMMON_MSG_EDIT_POST') : $t('COMMON_MSG_SAVE_POST')" @no='confirmNo(), checkPopYn=false' v-if="checkPopYn" @ok='sendMsg(), checkPopYn=false' />
            ...
            <div class="overFlowYScroll boardInputArea">
              ...
              <div style="width: 100%; height: calc(100% - 39px); float: left; padding-top:1.5rem; overflow: hidden auto; ">
                ...
                <div class="writeBoardPageTopArea font15">
                  <div class=""><p class="boardWriteTitleText font15 fontBold commonColor" style="">{{ $t('COMMON_TITLE_TITLE') }}</p><input type="text" id="pushTitleInput" placeholder="제목을 입력해주세요" class="font15 pageTopInputArea font15 inputArea fl" v-model="writePushTitle" style="background-color:white" name="" ></div>
                  <div class="" v-if="propData.nonMemYn"><p class="font15 boardWriteTitleText" style="">{{$t('COMMON_TITLE_CREATOR')}}</p><input type="text" id="pushTitleInput" placeholder="이름을 입력해주세요" class="pageTopInputArea font15 inputArea fl" v-model="nonMemUserName" style="background-color:white" name="" ></div>
                  <div v-show="fileYn" style="float: left; width: 100%; padding: 10px 0; padding-top: 0; min-height: 50px;">
                    <div style="width: 100%; min-height: 30px;">
                      ...
                      <p class="boardWriteTitleText fontBold font15 fl commonColor" style="margin-top: 4px;">{{$t('COMM_NAME_CONT_FILE')}}</p>
...
export default {
  ...
  methods: {
    ...
    async selectBoard (data, index) {
      ...
      if (mCabinetShare[0]) {
        if (mCabinetShare[0].shareType) {
          ...
          this.writeBoardPlaceHolder = this.$t('COMM_MSG_NO_PERM')
        }
      } else {
        this.selectBoardCabinetKey = null
        this.writeBoardPlaceHolder = this.$t('COMM_MSG_NO_PERM')
    ...
    async getTeamMenuList () {
      var paramMap = new Map()
      paramMap.set('teamKey', this.propData.targetKey)
      paramMap.set('currentTeamKey', this.propData.targetKey || this.propData.targetKey)
      paramMap.set('sysCabinetCode', 'BOAR')
      paramMap.set('shareType', 'W')
      paramMap.set('userKey', this.GE_USER.userKey)
      // console.log(paramMap)
    ...
    async clickPageTopBtn () {
      if (this.viewTab === 'complex' && this.complexOkYn === false) {
        ...
        if (title !== undefined && title !== null && title !== '') {
          ...
          this.errorText = this.$t('COMM_MSG_PUT_TITLE')
          this.failPopYn = true
          this.complexOkYn = false
          return
        }
        ...
        if ((msgData !== undefined && msgData !== null && msgData !== '') || this.uploadFileList.length > 0) {
          ...
          this.errorText = this.$t('COMM_MSG_PUT_BOARD')
          this.failPopYn = true
          this.complexOkYn = false
          return
        }
