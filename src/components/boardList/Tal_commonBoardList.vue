      ...
      <template id="boardRow" v-for="(board, index0) in commonBoardListData" :key="index0">
        ...
        <!-- <div v-if="board.readYn === 0" class="readYnArea"></div> -->
          ...
          <!-- <p class="font18 fontBold commonColor">{{this.$makeMtextMap(alimDetail.userDispMtext).get('KO').chanName}}</p> -->
            ...
            <p class="font12 fl lightGray" @click="userNameClick(board.creUserKey, board.targetKey, board.blindYn === 1)" >{{blindYn === true ? $t('COMMON_NAME_UNKNOWN') : (board.showCreNameYn === 1? this.$changeText(board.creUserName): '')}}</p>
          ...
          <div id="alimCheckArea">
            <div class="alimCheckContents">
              ...
              <div v-if="nonMemYn !== true" class="fr">
                  <p class="commonBlack font12"  style="float: right;">{{t('COMMON_TAB_LIKED')}} {{board.likeCount}}개</p>
                  <p class="commonBlack font12" style="float: right; margin-right: 10px;">{{$t('COMMON_TODO_COMMENT')}} {{board.memoCount}}개</p>
              </div>
              <div v-else class="commonBlack font12" style="float: right; padding: 2px 10px; background: rgb(0 0 0 / 21%); border-radius: 5px;">{{board.memoCount > 0? $t('COMMON_TITLE_REPLYCOM') : $t('COMMON_TITLE_REPLYWAIT')}}</div>
            </div>
          </div>
        </div>
      </template>
