...
export default {
  ...
  methods: {
    ...
    async readyFunc () {
      this.chanDetail = await this.$getDetail('TEAM', this.chanAlimListTeamKey)[0]
    }
  },
  data () {
    return {
  ...
  watch: {
    ...
    CHANNEL_DETAIL () {
      console.log(this.CHANNEL_DETAIL.blackYn)
    }
  },
  computed: {
    CHANNEL_DETAIL () {
      // console.log(this.chanDetail)
      if (this.chanAlimListTeamKey) {
        var detail = this.$getDetail('TEAM', this.chanAlimListTeamKey)[0]
        console.log('***************************')
        console.log(detail)
        // eslint-disable-next-line vue/no-side-effects-in-computed-properties
        this.bgblack = false
        if (detail && (detail.blackYn !== undefined && detail.blackYn !== null && detail.blackYn !== '')) {
          // eslint-disable-next-line vue/no-side-effects-in-computed-properties
          if (detail.blackYn === 1) this.bgblack = true
        }
        return detail
      } else {
        return null
