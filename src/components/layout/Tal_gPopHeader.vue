<template>
  <div class="commonPopHeaderWrap " style="" :class="{ 'newHeaderLine' : !chanAlimListTeamKey && targetType !== 'createChannel' && targetType !== 'contentsDetail' && targetType !== 'boardMain' , 'headerShadow' : chanAlimListTeamKey}" :style="targetType === 'chanInfo' ? 'background:transparent !important;' : ';' + 'padding-top:' + (this.$STATUS_HEIGHT + 20)+ 'px;' + 'padding-bottom: 1.7rem'">
    ...
    <span v-if="$route.path !== '/todo' || headerTitle.includes('일정')" class="popHeaderTitleSpan font20" :class="{colorBlack : (this.headerTitle === '게시판 작성')|| this.targetType === 'contentsDetail'}" :style="bgblack === true ? 'color:white;':'' ">
      {{this.$changeText(headerTitle)}}
    </span>
    <span v-else class="popHeaderTitleSpan font20" :class="{colorBlack : (this.headerTitle === '게시판 작성')|| this.targetType === 'contentsDetail'}" :style="bgblack === true ? 'color:white;':'' ">
    </span>
    <!-- 멤버 도우미 버튼 -->
    <!-- helpYn으로 변경해야 함 -->
...
.popHeaderTitleSpan{position: absolute; color: rgb(74 102 158); font-weight: bold; width:70%; text-overflow: ellipsis; white-space: nowrap; overflow: hidden;}
