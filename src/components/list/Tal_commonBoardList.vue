...
export default {
  data: function () {
    return { // 데이터 정의
      ...
      userKey: this.$store.getters['D_USER/GE_USER'].userKey
    }
  },
  props: {
    resultSearchKeyList: {},
...
async changeAct (act, contentsKey, idx) {
      ...
      param.userName = this.$changeText(this.$store.getters['D_USER/GE_USER'].userDispMtext || this.$store.getters['D_USER/GE_USER'].userNameMtext)
      if (saveYn === false) {
