        ...
        <tr v-for="(value, index) in commonListData.slice(0,5)" class="commonListTr textLeft " :style="index === commonListData.length - 1 ? 'border: none!important;' : ''" :key="index" @click="clickInfo(value)">
            ...
            <td v-on:click="goChanDetail(value)" :class="{top5MyPushColor:  sessionUserKey === value.creUserKey}">
                <div style="width:100%; float: left; padding: 2px 0 ; min-height: 25px;">
                    ...
                    <p v-if="value.jobkindId === 'BOAR' && !(this.$checkUserAuth(value.shareItem).V === true || value.creUserKey === this.GE_USER.userKey ) && (value.titleBlindYn === true || value.titleBlindYn === 1)" class="commonBlack textOverdot font15 fontBold" style="width: calc(100% - 75px); display: inline-block; white-space: nowrap; text-overflow: ellipsis;overflow: hidden;">{{ $t('COMM_MSG_NOPERM') }}</p>
