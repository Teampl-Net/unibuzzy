...
<template>
  <div id="addTeamMemberArea" class="addTeamMemberArea">
    ...
    <userImgSelectCompo
      @closeXPop="closeXPop"
      :pSelectedIconPath="mUserInfo.domainPath + mUserInfo.userProfileImg"
      :parentSelectedIconFileKey="mUserInfo.picMfilekey"
      @no="backClick"
      v-if="changeUserIconShowYn"
    />
    <div
      class="w100P fl mbottom-1 box1"
      :style="`padding-top: ${$STATUS_HEIGHT + 50}px;`"
    >
      <div
        class="flexCenter"
        :style="
          'width: ' + popSize * 0.3 + 'px; height: ' + popSize * 0.3 + 'px;'
        "
      >
        <div
          :style="
            'background-image: url(' +
            (mUserInfo.domainPath
              ? mUserInfo.domainPath +
                $changeUrlBackslash(mUserInfo.userProfileImg)
              : mUserInfo.userProfileImg) +
            '); width: ' +
            popSize * 0.3 +
            'px; height: ' +
            popSize * 0.3 +
            'px;'
          "
          class="userProfileImgWrap certiIconBox"
        >
          <img
            v-if="GE_USER.certiDate"
            class="img-w38"
            src="@/assets/images/common/userCertiIcon.svg"
            alt=""
          />
        </div>
      </div>
      <div v-if="selfYn" @click="changeUserImg()" class="font14 myIconBtn">
        변경
      </div>
    </div>
    <div class="addMemberTextArea fl">
      <div class="fl w100P dispContents">
        <p
          class="fl commonBlack creChanInput w100P font16 fontBold"
          v-if="readOnlyYn && !changeYn"
        >
          {{ memName }}
        </p>
        <p
          class="fl commonGray creChanInput w100P font14"
          v-if="readOnlyYn && !changeYn && GE_USER.certiDate"
        >
          {{ $changeText(mUserInfo.userDispMtext) }}
        </p>
        <img
          v-if="readOnlyYn && !changeYn && selfYn"
          src="@/assets/images/contents/noticebox_edit.png"
          class="fr cursorP noticeEditBtn"
          @click="changeUserDispMtext()"
        />
        <div v-show="changeYn" class="fl creChanInput" style="">
          <input
            class="fl font16 nameInput"
            type="text"
            v-model="memName"
            @keyup.enter="setDispName"
          />
          <div class="fl w100P">
            <p class="fl mleft-1 font13 height30" @click="setDispName">
              {{ $t('COMMON_BTN_OK') }}
            </p>
            <p class="fl mleft-1 font13 height30" @click="changeYn = false">
              {{ $t('COMM_BTN_CANCEL') }}
            </p>
          </div>
        </div>
      ...
      <div
        class="mtop-1 fl w100P flexJustiCenter"
        v-if="!readOnlyYn && !selfYn"
      >
        <img
          src="/resource/footer/icon_people.svg"
          class="img-w20 fl mright-05"
          alt=""
        />

        <input
          type="text"
          :placeholder="$t('MEM_DETAIL_MSG_NAME')"
          class="creChanInput fr"
          v-model="memName"
        />
      </div>
      ...
      <div class="mtop-1 fl w100P flexJustiCenter">
        <img
          src="@/assets/images/editChan/icon_letter.svg"
          class="img-w20 fl mright-05"
          alt=""
        />
        <p
          class="fl font16 commonDarkGray creChanInput textLeft height30"
          v-if="readOnlyYn"
        >
          {{ memEmail ? memEmail : $t('MEM_DETAIL_MSG_NOMAIL') }}
        </p>
        <input
          v-else
          type="text"
          :placeholder="$t('MEM_DETAIL_MSG_EMAIL')"
          class="creChanInput fr"
          v-model="memEmail"
        />
      </div>
      <div class="mtop-1 fl w100P flexJustiCenter">
        <img
          src="@/assets/images/editChan/icon_phoneSolid.svg"
          class="img-w20 fl mright-05"
          alt=""
        />
        <p
          ...
          v-if="readOnlyYn"
        >
          {{ memPhone ? memPhone : $t('MEM_DETAIL_MSG_NOPHONE') }}
        </p>
        <input
          v-else
          type="text"
          :placeholder="$t('MEM_DETAIL_MSG_PHONE')"
          class="creChanInput fr"
          @keyup.enter="addDirectAddMemList"
          v-model="memPhone"
        />
      </div>
      ...
      <gBtnSmall
        ...
        class="fl btnStyle"
        @click="addDirectAddMemList"
      />
      <div v-if="readOnlyYn" class="fl w100P mtop-3 actBox">
        <div
          v-for="(value, index) in profileFunc"
          :key="index"
          @click="profileFuncEvent(value.type)"
          class="fl flexCenter"
        >
          <div class="flexAlignCenter flexColumn">
            <div class="nativeServiceBtnWrap">
              <img
                v-if="value.type === 'MAIL'"
                src="@/assets/images/editChan/icon_letter.svg"
                class="img-w20"
                alt=""
              />
              <img
                v-if="value.type === 'PHON'"
                src="@/assets/images/editChan/icon_phoneSolid.svg"
                class="img-w20"
                alt=""
              />
              <img
                ...
                src="@/assets/images/editChan/icon_textSolid.svg"
                class="img-w20"
                alt=""
              />
            </div>
            <p class="font14 fl textLeft commonBlack height30">
              {{ value.funcTitle }}
            </p>
          </div>
          <div
            class="mleft-05 mright-05 whiteColor"
            v-if="index !== profileFunc.length - 1"
          >
            |
          </div>
        </div>
      ...
      <div>
        <table class="memberTable">
          ...
          >
            ...
            <td class="font16 memList fontBold" @click="deleteMem(data, index)">
              X
            </td>
          </tr>
        </table>
...
export default {
  data() {
    return {
      ...
      tempUserDispName: '',
      profileFunc: [
        { funcTitle: '메일쓰기', type: 'MAIL' },
        { funcTitle: '전화걸기', type: 'PHON' },
        { funcTitle: '문자쓰기', type: 'TEXT' }
      ],
      mUserInfo: {}
    }
  },
