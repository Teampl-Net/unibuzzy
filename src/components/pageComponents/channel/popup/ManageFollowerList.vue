...
<template>
  <div
    class="w100P h100P mngFollowerListWrap"
    :style="'padding-top:' + ($STATUS_HEIGHT + 90) + 'px'"
  >
    <gPopHeader
      :headerTitle="$t('FOLLOW_BTN_MANA_MEM')"
      :pClosePop="pClosePop"
    />
    <div
      class="pageTopAreaStyle"
    ...
    <div class="w100P fl memberSearchArea">
      <div class="fl memberSearchBox">
        ...
        <input
          ...
          :placeholder="$t('FOLLOW_MSG_USERINFO')"
        />
      </div>
      <div class="CDeepBorderColor fr memberOrderBox">
        ...
        >
          {{ $t('FOLLOW_BTN_ORDER_NAME') }}
        </p>
      </div>
    </div>
    <div class="w100P memberListContentsBox">
      <followerItem
        :managingList="GE_DISP_MANAGER_LIST"
        @saveManager="saveManager"
        :memberYn="tab === 'M' ? true : false"
        @openPop="openPop"
    ...
    />
    <gConfirmPop
      @no="timerPopShowYn = false"
      :confirmText="$t('FOLLOW_MSG_SUCCESS')"
      confirmType="timeout"
      v-if="timerPopShowYn"
    />
  </div>
...
export default {
  ...
  methods: {
    ...
    async getFollowerList() {
      ...
      if (
        this.searchFilterObj.userDispMtext != null &&
        this.searchFilterObj.userDispMtext !== ''
      ) {
        paramMap.set('userDispMtext', this.searchFilterObj.userDispMtext)
      }
      if (
        this.searchFilterObj.phoneEnc != null &&
        this.searchFilterObj.phoneEnc !== ''
      ) {
        paramMap.set('phoneEnc', this.searchFilterObj.phoneEnc)
      }
      if (
        this.searchFilterObj.userEmail != null &&
        this.searchFilterObj.userEmail !== ''
      ) {
    ...
    replaceArr(arr) {
      var uniqueArr = arr.reduce(function (data, current) {
        if (
          data.findIndex(({ userKey }) => userKey === current.userKey) === -1
        ) {
          data.push(current)
        }
        return data
  ...
  computed: {
    ...
    GE_DISP_SHOW_PROFILE_USER_LIST() {
      for (var i = 0; i < this.showUserList.length; i++) {
        ...
        var idx1 = dataList.findIndex(
          (item) => item.followerKey === this.showUserList[i].followerKey
        )
        if (idx1 !== -1) {
    ...
    GE_DISP_MANAGER_LIST() {
      for (var i = 0; i < this.managerList.length; i++) {
        ...
        var idx1 = dataList.findIndex(
          (item) => item.managerKey === this.managerList[i].managerKey
        )
        if (idx1 !== -1) {
  ...
  watch: {
    GE_LOCALE: {
      ...
      handler(value) {
        this.activeTabList = [
          { display: this.$t('COMMON_TAB_ALL'), name: 'A' },
          { display: this.$t('COMMON_NAME_MANAGER'), name: 'AD' }
        ]
      },
      deep: true
    },
    ...
    GE_NEW_SHOW_LIST: {
      handler(value, old) {
        ...
        var newArr = [...this.showUserList, value[0]]
    ...
    GE_NEW_MAN_LIST: {
      handler(value, old) {
        ...
        var newArr = [...this.managerList, value[0]]
