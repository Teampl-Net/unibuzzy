...
export default {
  ...
  methods: {
    ...
    callbackF (response) {
      ...
      if (success) {
        this.$showToastPop('본인인증 성공')
      } else {
        this.$showToastPop(`본인인증 실패: ${error_msg}`)
      }
    },
    async checkIdentity () {
      ...
      // 패치해라
      const returnSuccessUrl = 'https://www.hybric.net:9443/service/tp.successUserCertification'
      const returnErrorUrl = 'https://www.hybric.net:9443/service/tp.successUserCertification'
      ...
      var res = await this.$commonAxiosFunction({
        url: 'https://www.hybric.net:9443/service/tp.getNiceToken',
