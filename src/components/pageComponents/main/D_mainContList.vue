<template>
  <div style="width: 100%; min-height: 100px; float: left; display: flex; flex-direction: column; justify-content: center; align-items: center; padding-bottom: 40px; " :key="mReloadKey" >
        <template v-for="(cont, index) in this.GE_DISP_CONTS_LIST" :key="cont.contentsKey" >
            <gContentsBox :pOpenUnknownLoginPop="pOpenUnknownLoginPop" :pUnknownYn="pUnknownYn" class="mBottom-05" ref="myContentsBox" @requestSearchSticker="requestSearchSticker" @openImgPop="openImgPop" :imgClickYn="true" :propDetailYn="false" :contentsEle="cont" @openPop="openPop" :propContIndex='index' @contDelete='contDelete' />
  ...
  methods: {
    ...
    requestSearchList (data) {
      ...
      if (data.searchKey) {
        ...
        index = mSearchList.findIndex(item => item.searchType === this.$t('COMMON_TITLE_TITLE'))
        if (index !== -1) {
      ...
      if (data.creTeamNameMtext) {
        ...
        index = mSearchList.findIndex(item => item.searchType === this.$t('COMMON_TITLE_CHANNAME'))
        if (index !== -1) {
          mSearchList.splice(index, 1)
        }
        mSearchList.push(searchObj)
      ...
      if (data.creUserName) {
        ...
        index = mSearchList.findIndex(item => item.searchType === this.$t('COMMON_TITLE_CREATOR'))
        if (index !== -1) {
          mSearchList.splice(index, 1)
        }
        mSearchList.push(searchObj)
      ...
      if (data.fromCreDateStr && data.toCreDateStr) {
        ...
        index = mSearchList.findIndex(item => item.searchType === this.$t('COMMON_TITLE_DATE'))
        if (index !== -1) {
          mSearchList.splice(index, 1)
        }
        mSearchList.push(searchObj)
      ...
      if (data.selectedSticker) {
        ...
        index = mSearchList.findIndex(item => item.searchType === this.$t('COMMON_TITLE_CATEGORY'))
        if (index !== -1) {
          mSearchList.splice(index, 1)
        }
        mSearchList.push(searchObj)
    ...
    async getMyContentsList (pageSize, offsetInput, loadingYn) {
      ...
      if (this.pMyCabinetKeyListStr) {
        param.cabinetKeyListStr = this.pMyCabinetKeyListStr
      }
      ...
      if (this.mSelectedStickerKey != null) {
        ...
        param.stickerKeyList = [this.mSelectedStickerKey]
      }
      var nonLoading = true
      if (loadingYn) {
        nonLoading = false
