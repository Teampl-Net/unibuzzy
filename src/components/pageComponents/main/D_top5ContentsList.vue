...
export default {
  ...
  created () {
    // console.log('propAlimList 최근 컨텐츠', this.propAlimList)
  },
  methods: {
    replaceArr (arr) {
    ...
    GE_LOCALE: {
      ...
      handler (value) {
        if (this.pListTitle === 'noti') {
          ...
          this.mActiveTabList = [{ display: this.$t('COMMON_TAB_LIKED'), name: 'L' }, { display: this.$t('COMMON_TAB_SAVED'), name: 'S' }]
        }
      },
      deep: true
    },
