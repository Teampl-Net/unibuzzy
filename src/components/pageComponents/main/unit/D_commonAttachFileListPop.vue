  ...
  <div class="" style="width: 80%; word-break: break-all; box-shadow: 0px 0px 8px rgba(0, 0, 0, 0.2); border-radius: 6px 6px 6px 6px; left: 10%; top: 20%; background: #fff; z-index: 21; position: fixed;">
    <div style=" margin: 15px; float: left; width: calc(100% - 30px); position: relative; " >
      ...
      <div class="fl w-100P thinScrollBar" style="padding-left:0.5rem; min-height: 150px; max-height: 250px; height: 35%; overflow: hidden auto;">
        <p class="fl font14 fontBold w-100P textLeft" v-if="mAttachFileList.D_ATTACH_FILE_LIST && mAttachFileList.D_ATTACH_FILE_LIST.length > 0">첨부파일</p>
        ...
        <p class="fl font14 fontBold w-100P textLeft mtop-05" v-if="mAttachFileList.D_BODY_IMG_FILE_LIST && mAttachFileList.D_BODY_IMG_FILE_LIST.length > 0">본문 이미지</p>
  ...
  methods: {
    ...
    async selectAttachedFile (myFilekey, value, index, fileType) {
      ...
      var result = await this.$commonAxiosFunction({
        url: 'https://www.hybric.net:9443/service/tp.saveMyFile',
        param: { file: file }
      })
      if (fileType === 'F') {
