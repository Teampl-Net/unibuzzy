    ...
    <div id="boardDetailScrollArea" class="pagePaddingWrap root mtop-1 overflowYScroll" ref="memoarea" >
      <div class="content pushMbox" v-for="(alim, index) in alimDetail" :key="index" :change="changeData">
        <div class="pushDetailTopArea">
          <div @click="goChanDetail(alim)"  class="boardDetailChanLogoImgWrap cursorP fl" :style="'background-image: url(' + (alim.domainPath ? alim.domainPath + alim.logoPathMtext : alim.logoPathMtext) + ');'" style="background-repeat: no-repeat; background-size: cover; background-position: center;">
          </div>
...
<!-- :class="{commonBlue: alim.readYn === 0}"  -->
                  ...
                  <div class="w-100P fl">
                    <p style="width:65%; " class="font14 fl grayBlack">
                      ...
                      <pp @click="userNameClick(alim.creUserKey, alim.creTeamKey, alim.showCreNameYn === 0)">{{alim.showCreNameYn === 0 ? '(익명)' : (alim.showCreNameYn === 1? '(' + this.$changeText(alim.creUserName) + ')': '')}}</pp>
                    </p>
        ...
        <div id="alimCheckArea">
          ...
          <div v-if='!alim.canReplyYn' class="fl w-100P mtop-05 mbottom-05" style="background-color:#cccccc50; padding: 0.5rem 0; border-radius: 10px;">
            <p class="w-100P commonBlack font13 textCenter" >작성자가 댓글 사용을 중지하였습니다.</p>
          </div>
...
import manageStickerPop from '../../popup/sticker/Tal_manageStickerPop.vue'
export default {
  data () {
    return {
  ...
  methods: {
    ...
    decodeContents (data) {
      ...
      // this.$inputChangeText(changeText)
      // alert(true)
      return changeText
    },
    ...
    pointAni (memoKey) {
      var firstMemoCard = document.querySelectorAll('#memoWrap #' + memoKey)[0]
      if (firstMemoCard) {
