...
export default {
  ...
  data () {
    return {
      ...
      mTodoInputList: [
        ...
        { title: 'Tag', value: 'A', options: [{ opt: 'Memo', value: 'H' }, { opt: 'Work', value: 'T' }, { opt: 'Idea', value: 'S' }, { opt: 'etc', value: 'E' }] },
  ...
  methods: {
    saveTodo () {
      if (this.pSaveTodo) {
        if (this.mTodoTitle === '') {
          this.$showToastPop('Please write a title.')
        } else if (this.mTodoCate === 'A') {
          this.$showToastPop('Please select a category.')
        } else {
          const todoSet = {}
