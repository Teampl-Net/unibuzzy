...
export default {
  ...
  created () {
    if (this.GE_MAIN_CHAN_LIST) {
      var result = this.GE_MAIN_CHAN_LIST
      console.log('result', result)
    }
    // console.log('pChanDetail', this.pChanDetail)
    // if (this.pChanDetail) {
    //   this.mContsList = this.pChanDetail[0].ELEMENTS.alimList
    //   console.log('this.mContsList', this.mContsList)
    // }
  ...
  methods: {
    ...
    async requestSearchList (param) {
      ...
      console.log('requestSearch - resultList', resultList)
    ...
    async getTownCabinetList () {
      if (this.pTownTeamKey) {
        ...
        var result = await this.$commonAxiosFunction({
          url: 'https://www.hybric.net:9443/service/tp.getTownCabinetList',
          param: param
        })
    ...
    async setContsList (resultList) {
      ...
      this.mShowSkeletonYn = false
      console.log('setContsList - mContsList', this.mContsList)
    },
