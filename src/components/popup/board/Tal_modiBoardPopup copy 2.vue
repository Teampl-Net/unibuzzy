...
export default {
  ...
  methods: {
    ...
    // 선택한 상위 권한리스트와 하위리스트를 비교하여 제거(?)해주는 함수
    topListVSbottomList (topList, bottomList) {
      var tempList = []
      for (let i = 0; i < bottomList.length; i++) {
        var idx = topList.findIndex(item => item.accessKey === bottomList[i].accessKey)
        if (idx !== -1) {
          tempList.push(bottomList[i])
        }
      }
    ...
    receiverPoolInSetting (datas) {
      ...
      if (datas.bookList) {
        ...
        if (this.currentSelectBookType === 'V') {
          if (!this.permissionVGroup.selectedList.bookList) {
            this.permissionVGroup.selectedList.bookList = []
          }
          // this.permissionVGroup.selectedList.bookList = []
        ...
        } else if (this.currentSelectBookType === 'W') {
          if (!this.permissionWGroup.selectedList.bookList) {
            this.permissionWGroup.selectedList.bookList = []
          }
          // this.permissionWGroup.selectedList.bookList = []
