  ...
  <div class="channelMenuWrap showModal-enter" :class="{editWrap: editYn === true, 'showModal-leave': closeYn === true  }" style="overflow: hidden scroll;">
    ...
    <div v-if="true" class="fl w-100P" style="overflow: hidden scroll;">
      <div v-if="CHANNEL_DETAIL.D_CHAN_AUTH.ownerYn || CHANNEL_DETAIL.D_CHAN_AUTH.adminYn" class="fl w-100P" style="margin-top:50px;" >
        ...
        <div class="boardBox boardBoxDown" style="overflow: hidden scroll; width: calc(100% - 100px); clear:left " ref="addressBookGroupRef" :class="{boardBoxUp: bookDropDownYn === false, boardBoxDown: bookDropDownYn === true}" >
      ...
      <div class="fl w-100P mtop-2" :style="(CHANNEL_DETAIL.D_CHAN_AUTH.ownerYn || CHANNEL_DETAIL.D_CHAN_AUTH.adminYn) ? 'margin-top: 1rem !important;' : 'margin-top:calc(50px + 1rem);'" >
        ...
        <div class="boardBox boardBoxDown" style="overflow: hidden scroll; width: calc(100% - 100px); clear:left " ref="boardRef" :class="{boardBoxUp : boardDropDownYn === false, boardBoxDown:boardDropDownYn === true}" >
      ...
      <div>
        <p class="fl font14 cursorP commonColor fontBold mtop-07" style="white-space: nowrap;" @click="convenienceFuncDropdown">
        ...
        <div class="boardBox boardBoxDown" style="overflow: hidden scroll; width: calc(100% - 100px); clear:left " ref="convenienceFunction" :class="{boardBoxUp : convenienceFuncYn === false, boardBoxDown:convenienceFuncYn === true}" >

          <div v-for="(data, index) in convenienceFuncList" :key="index" @click="convenienceFunc(data.targetType)" class="MenuListNew fl cursorP" style="width:100%;">
            ...
            <div class="fl mleft-2 textLeft font16 textOverdot" style="width: calc(100% - 30px - 3rem); margin: 0 0 0 0; width:100% !important;" v-if="data.targetType !== 'writePush' || (data.targetType === 'writePush' && (CHANNEL_DETAIL.D_CHAN_AUTH.adminYn === true || CHANNEL_DETAIL.D_CHAN_AUTH.ownerYn === true)) " >
              <span class="commonBlack fontBold mleft-05">{{data.title}}</span>
            </div>
          </div>
        </div>
      </div>
