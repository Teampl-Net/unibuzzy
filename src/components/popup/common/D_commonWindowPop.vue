...
export default {
  ...
  methods: {
    openWinPop (uri, width, height) {
      ...
      // 1. 윈도우 팝업 띄우기
      try {
        this.windowRef = window.open('about:blank', 'popupChk', 'width=500, height=550, top=100, left=100, fullscreen=no, menubar=no, status=no, toolbar=no, titlebar=yes, location=no, scrollbar=no')
        if (this.windowRef != null) {
