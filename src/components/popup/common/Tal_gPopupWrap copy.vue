<template>
    <div id="gPopup" v-if="reloadYn === false && this.popId" :style="this.targetType === 'writeContents'? 'background: transparent' : ''" class="commonPopWrap">
      ...
      <transition name="showModal">
        <fullModal @goScrollTarget="goScrollTarget" @successWrite="successWriteBoard" @parentClose="parentClose" @addDirectAddMemList="addDirectAddMemList" @reloadPop="reloadPop" :style="getWindowSize" transition="showModal" :id="popId" ref="commonGPopWrap" @selectedReceiverBookNMemberList='selectedReceiverBookNMemberList' @closePop="closePop" v-if="this.popShowYn" :parentPopN="this.thisPopN" :propParams="this.popParams" :propData="this.propParams" @toAlimFromBoard='toAlimThisPageClose' @saveCabinet='refreshCabinet' @channelMenuReload='channelMenuReload' @closeNewPop='closeNewPop'                                        />
      </transition>
...
export default {
      ...
      if (history.length < 2 && (history[0] === 0 || history[0] === undefined)) {
        ...
        if (value.length > 0) {
          ...
          if (target.targetKind === 'chanDetail') {
            this.getChanDetailAndOpenPop(param)
          } else {
            this.getContsDetailAndOpenPop(param)
          }
  ...
  methods: {
    ...
    async setGPopPropData (successChanYn) {
      ...
      if (this.targetType === 'contentsDetail' || this.targetType === 'chanDetail') {
        ...
        if (target.jobkindId === 'QUES') this.headerTitle = '문의하기'
      ...
      if (this.parentPopN !== undefined && this.parentPopN !== null && this.parentPopN !== '') { this.thisPopN = Number(this.parentPopN) + 1 } else { this.thisPopN = 100 }
      if (!this.popId) { this.popId = 'gPopup' + this.thisPopN }
    ...
    openPop (params) {
      if (params.targetType === 'chanDetail') {
        this.getChanDetailAndOpenPop(params)
        return
      } else if (params.targetType === 'contentsDetail') {
        this.getContsDetailAndOpenPop(params)
        return
      }
    ...
    async getChanDetailAndOpenPop (detailValue) {
    ...
    async getContsDetailAndOpenPop (detailValue) {
      // var history = this.$store.getters['D_HISTORY/hStack']
      ...
      if (detailValue.jobkindId === 'BOAR') { axiosParam.popHeaderText = result.cabinetNameMtext } else { axiosParam.popHeaderText = result.nameMtext }
      // if (!axiosParam.popHeaderText) axiosParam.popHeaderText = detailValue.popHeaderText
      axiosParam.targetType = 'contentsDetail'
      axiosParam.creTeamKey = Number(teamKey)
      axiosParam.notiYn = true
      axiosParam.value = detailValue
      axiosParam.initData = result
      if (result) {
        this.popParams = axiosParam
        this.popShowYn = true
      }
    },
    ...
    async recvNotiFromMain (notiDetail, currentPop, vuexResultData) {
      ...
      if (notiUserDo.targetKind === 'T') {
        this.getChanDetailAndOpenPop(goDetailParam)
      } else if (notiUserDo.targetKind === 'C' || notiUserDo.targetKind === 'B') {
        this.getContsDetailAndOpenPop(goDetailParam)
      }
    },
    async recvNoti (e) {
      // var message
