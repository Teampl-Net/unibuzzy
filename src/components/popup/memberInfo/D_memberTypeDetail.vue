<template>
    <div v-if="mMemberTypeDetail" class="fl wh-100P" :style="innerWidth" style="white-space:nowrap; display: inline; padding-top:1.5rem; min-width: var(--minwidth); position: relative; ">
      ...
      <div class="fl mtop-1" :key="reloadKey" style="padding: 0 10px" :style="propLeftYn === false ? 'width:100%': ''">
        ...
        <mTypeQueList ref="mTypeQueList" :propMemberTypeObj="mMemberTypeDetail" @addQuestion="this.$emit('addQuestion')" @editQue="editQue" @deleteQue="deleteQue" class="mtop-1" />
      </div>
...
export default {
  ...
  methods: {
    ...
    deleteQue (memberTypeItemObj) {
      console.log(memberTypeItemObj)
      if (memberTypeItemObj.targetType === 'deleteQue') {
        this.$emit('deleteQue', memberTypeItemObj)
        console.log(memberTypeItemObj)
      }
    },
