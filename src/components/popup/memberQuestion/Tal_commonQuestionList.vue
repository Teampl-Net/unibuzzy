<template>
  <draggable @end="changePosTeamMenu" handle=".draggablePointer" class="ghostClass" v-model="questionList" ghost-class="ghost" style="padding-top: 10px; --webkit-tap-highlight-color: rgba(0,0,0,0);" :disabled='enabled' delay="200"  >
    <transition-group>
      <div v-for="(data, index) in questionList" :key="index" class="fl w-100P mtop-05 mbottom-05" style="border-radius:10px; background:white; padding:1rem 1rem 1rem 0; box-shadow: 0px 4px 9px 0px #00000040; position: relative;">
        ...
        <div style="width:calc(100% - 30px);" class="fr">
          ...
          <div class="questionMiddleArea">
            <div v-if="data.questionType === 'su' || data.questionType === 'at'" class="w-100P fl mtop-05" style="width:100%; background:white; border: 1px dashed #ccc; padding-left:0.5rem;" :style="data.questionType === 'at' ? 'padding: 1rem 0;' : 'background-color:#eeeeeeee'">
              ...
              <img v-if="data.questionType === 'at'" style="width:20px;" src="../../../assets/images/channel/Image_icon.svg" w alt="">
            </div>
            <div v-if="data.questionType === 'si' || data.questionType === 'mu'" class="w-100P fl mtop-05 textLeft" >
              ...
              <p @click="addAnswerClick(data.answerList, index)" class="fl commonColor font14 fontBold mtop-05 mleft-12">+ 추가</p>
            </div>

          </div>
          <div class="questionBottomArea fl w-100P mtop-1" >
            <img style="width:15px;" class="fl" src="../../../assets/images/channel/Copy_icon.svg" alt="">
            <img style="width:20px;" class="fr" src="../../../assets/images/channel/Trash_icon.svg" alt="">
...
export default {
  ...
  methods: {
    addAnswerClick (list, index) {
      var temp = {}
      temp.answerName = '답안'
      this.questionList[index].answerList.push(temp)
    }
  }
}
</script>
