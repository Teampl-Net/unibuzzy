...
<div class="pagePaddingWrap questionListWrap " >
  <div style="width: 100%; height: calc(30% - 60px); position: absolute; left: 0; padding: 1rem;">
    ...
    <div class="fl w100P mtop-05" style="">
  ...
  <div style="background-color:#f0ebf8; width:100%; height:70%; margin-top:1rem; position: absolute; bottom:0; left:0;padding-top:1rem; box-shadow: inset 0px 1px 5px 0px #ccc;" class="fl" >
    ...
    <transition-group>
      <div v-for="(list, index) in questionList" :key="index" @click="editCardSet(list, index)" class="fl w100P questionListCard" style="min-height:50px; margin-bottom:1rem" :style="index === (questionList.length-1) ? 'border-bottom: none;' : '' ">
        ...
        <div v-show="editIndex === index" class="fl w100P" style="padding :0 1rem 1rem 1rem;">
          ...
          <div v-if="questionOption === 'Multiple'" class="fl w100P" style="">
            <div class="fl "> <input type="checkbox" id="MultipleCheck"> <label for="MultipleCheck">옵션1</label></div>
            <p @click="addOption()" class="fl font14 w100P textLeft">추가..</p>
          </div>

          <div v-if="questionOption === 'Descriptive'" class="fl w100P" style="">
            <div class="w100P fl mtop-05" style=" max-width:50%; min-width:50px; min-height:30px; background-color:ghostwhite; border-bottom:1px dotted #ccc">
