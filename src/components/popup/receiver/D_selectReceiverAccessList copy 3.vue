...
<div style="height: 100vh; background-color:white; width:100%; z-index:9999; position:absolute; top:0; left:0">
    ...
    <div class="w-100P pagePaddingWrap" style="position:absolute; overflow: auto; padding-top:50px" >
      <div style="width: 100%; position: relative; float: left; height: calc(100% - 95px); overflow: auto;">
        ...
        <selectBookNMemberList ref="selectedMemberListCompo" v-if="detailOpenYn === true" :itemType="itemType" @addSelectList="addSelectList" :selectBookNList='memberList' :selectList='pSelecedList' @detail='detailOpen' />
      </div>
...
export default {
  ...
  mounted () {
    ...
    this.setMemberList()
    this.$nextTick(() => {
      this_.$refs.testCompo.upDatePage(this.pSelecedList)
    })
  },
  ...
  methods: {
    setMemberList () {
      ...
      // for (var i = 0; i < this.pSelecedList.memberList.length; i++) {
      //   this.pSelecedList.memberList[i].selectedYn = true
      //   var temp = {}
      //   temp = tempList[i]
      //   temp.accessKey = tempList[i].user
