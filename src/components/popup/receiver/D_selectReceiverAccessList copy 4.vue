...
<div style="height: 100vh; background-color:white; width:100%; z-index:9999; position:absolute; top:0; left:0">
    ...
    <div class="w-100P pagePaddingWrap" style="position:absolute; overflow: auto; padding-top:50px" >
      <div style="width: 100%; position: relative; float: left; height: calc(100% - 95px); overflow: auto;">
        <selectBookNMemberList v-if="detailOpenYn === false" ref="selectBookNMemberListCompo" :itemType="itemType" @addSelectList="addSelectList" :propData='propData' :selectBookNList='baseBookNMemberList' :selectList='pSelecedList' @detail='detailOpen' />
        <!-- <selectBookNMemberList v-if="detailOpenYn === true"  ref="selectedMemberListCompo" :itemType="itemType" @addSelectList="addSelectList" :selectBookNList='memberList' :selectList='pSelecedList' @detail='detailOpen' /> -->
      </div>
...
export default {
  ...
  created () {
    ...
    //   }
    // }
    this.pSelecedList = JSON.parse(JSON.stringify(this.selectList))
  },
  async mounted () {
    var this_ = this
    await this.getBookList()
    this.$nextTick(() => {
      this_.$refs.testCompo.upDatePage(this_.pSelecedList)
      this_.setSelectedList = JSON.parse(JSON.stringify(this_.selectList))
      // this_.$refs.selectBookNMemberListCompo.reload(false)
      // this_.$refs.selectBookNMemberListCompo.reload(true)
    })
  },
  ...
  data () {
    return {
      ...
      selectPopId: {},
      pSelecedList: {},
      baseBookNMemberList: { bookList: [], memberList: []}
    }
  },
  methods: {
    async getBookList () {
      ...
      console.log(this.pSelecedList)
      var temp
      for (let i = 0; i < tempList.length; i++) {
        var idx = this.pSelecedList.bookList.findIndex(item => item.accessKey === tempList[i].cabinetKey)
        if (idx !== -1) {
          temp = {}
          temp = tempList[i]
          temp.jobKindId = 'BOOK'
          temp.accessKey = tempList[i].cabinetKey
          temp.selectedYn = true
          this.baseBookNMemberList.bookList.push(temp)
        }
      }
      if (this.pSelecedList.memberList.length > 0) {
        for (let i = 0; i < this.pSelecedList.memberList.length; i++) {
          temp = {}
          temp = this.pSelecedList.memberList[i]
          temp.jobKindId = 'USER'
          temp.selectedYn = true
          this.baseBookNMemberList.memberList.push(temp)
        }
        // this.baseBookNMemberList.memberList = this.pSelecedList.memberList
      }
      // this.editBookSelectedList()
      //

      console.log(this.baseBookNMemberList)
    },
    ...
    async changeSelectedItem (data) {
      ...
      await this.editBookSelectedList()
      await this.editMemberSelectedList()
      this.pSelecedList = data
      this.setSelectedList = data
      // this.$refs.selectBookNMemberListCompo.delSelectList(data.delKey, data.type)
      this.$refs.selectBookNMemberListCompo.setParentSelectList()
    },
    editMemberSelectedList () {
      if (this.setSelectedList.memberList) {
        var changeList = this.setSelectedList.memberList
        for (var m = 0; m < this.baseBookNMemberList.memberList.length; m++) {
          this.baseBookNMemberList.memberList[m].selectedYn = false
          for (var c = 0; c < changeList.length; c++) {
            if (changeList[c].accessKey === this.baseBookNMemberList.memberList[m].userKey) {
              this.baseBookNMemberList.memberList[m].selectedYn = true
            }
          }
        }
      }
    ...
    editBookSelectedList () {
      ...
      if (this.setSelectedList.bookList) {
        var changeList = this.setSelectedList.bookList
        for (var m = 0; m < this.baseBookNMemberList.bookList.length; m++) {
          this.baseBookNMemberList.bookList[m].selectedYn = false
          for (var c = 0; c < changeList.length; c++) {
            if (changeList[c].accessKey === this.baseBookNMemberList.bookList[m].cabinetKey) {
              this.baseBookNMemberList.bookList[m].selectedYn = true
            }
          }
        }
      }
