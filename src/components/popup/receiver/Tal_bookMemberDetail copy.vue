...
<userImgSelectCompo @closeXPop="closeXPop" :pSelectedIconPath="this.domainPath + userProfileImg" :parentSelectedIconFileKey="picMfilekey"  @no="backClick" v-if="changeUserIconShowYn"/>
    ...
    <div class="addMemberTextArea">
        ...
        <div v-if="readOnlyYn && mobileYn" style="width: 100%; margin-top: 30px; min-height: 70px; display: flex; flex-direction: row; justify-content: space-around;">
            <div v-for="(value, index) in profileFunc" :key="index" class="fl" style="display: flex; flex-direction: row; align-items: center; justify-content: center">
...
export default {
    data () {
        return {
            ...
            tempUserDispName: '',
            thisUserKey: null,
            profileFunc: [{ funcTitle: '알림작성', targetType: 'writeContents', type: 'ALIM' },
                        { funcTitle: '메일쓰기', targetType: 'writeContents', type: 'MAIL' },
                        { funcTitle: '전화걸기', targetType: 'callphone', type: 'PHON' },
                        { funcTitle: '문자쓰기', targetType: 'sendphone', type: 'TEXT' }]
        }
    },
    components: {
        popUp,
