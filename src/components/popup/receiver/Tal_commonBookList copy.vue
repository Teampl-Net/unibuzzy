...
export default {
    ...
    methods:{
            ...
            if(this.selectedBookList) {
                for (var i = 0; i < this.addressBookList.length; i ++) {
                    ...
                    for (var s = 0; s < this.selectedBookList.length; s ++) {
                        // console.log(this.selectedBookList[s].accessKey + '??' + this.addressBookList[i].cabinetKey )
                        if (this.selectedBookList[s].accessKey === this.addressBookList[i].cabinetKey) {
                            this.addressBookList[i].selectedYn = true
                            this.checkSelectList.push(this.addressBookList[i])
                            // this.selectedBookList.push(this.addressBookList[i])
                            break
                        }
                    }
                }
        ...
        addSelectedList(data, index) {
            ...
            console.log('=====================')
            console.log(this.checkSelectList)
            console.log(data)
            console.log(this.addressBookList)
            console.log('=====================')
            var indexOf = this.checkSelectList.findIndex(i => i.cabinetKey === data.cabinetKey);
            if (indexOf === -1) {
                this.checkSelectList.push(data)
                this.addressBookList[index].selectedYn = true
                this.$emit('changeSelectBookList', this.checkSelectList)
            }else{
