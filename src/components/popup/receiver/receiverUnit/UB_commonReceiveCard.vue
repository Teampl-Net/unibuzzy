  ...
  <div class="fl w100P receiverItemWrap" :class="{ CWhiteGrayBgColor: (option === 'SELE' && propData.selectedYn === true) }" >
    <div class="fl w100P rowBaseCss">
      ...
      <template v-else-if="propData.jobkindId === 'USER'">
        <div class="imgCircle middleBgColor fl" @click="clickEvntToParents('open')" >
          <div v-if="propData.domainPath || propData.userProfileImg" :style="'background-image: url(' + (propData.domainPath? propData.domainPath + (propData.userProfileImg ? propData.userProfileImg : propData.pathMtext ) : propData.userProfileImg ) + ');'" class="memberPicImgWrap">
          </div>
