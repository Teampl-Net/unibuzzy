...
<template>
  <pre
    @focus="showToolBox"
    @keyup="changeFont"
    :placeholder="$t('FORM_MSG_CONTENTS')"
    contenteditable
    style="
    ...
    :formIdx="targetKey"
    ref="formTextArea"
    :id="'formTextArea' + targetKey"
    class="formCard formText editableContent formCardTextid formEditorTextPadding"
    @input="inputResize('formTextArea' + targetKey)"
  ></pre>
</template>
...
export default {
  ...
  mounted() {
    if (this.targetKey) {
      document
        .querySelector('#formCard' + this.targetKey)
    ...
    if (
      this.propsTools !== undefined &&
      this.propsTools !== 'undefined' &&
      this.propsTools !== null &&
      this.propsTools !== 'null' &&
      this.propsTools !== ''
    ) {
