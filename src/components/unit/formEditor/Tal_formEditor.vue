          ...
          <div class="fl" style="width: 100%; height: 100%; height: 100%;">
              <draggable id="dragCompp"  ref="editableArea" class="ghostClass" :v-model="formCardList" ghost-class="ghost" :dragging="dragging" @choose='test' @end="changePosTeamMenu" delay="200" handle=".movePoint">
                  <transition-group>
                      ...
                      <!-- A9AACD -->
                      <div v-for="(value, index) in formCardList" :index="index" :class="value.type === 'text' ? 'formCardBackground': 'formLineCard'" :style="this.selectRow === value.targetKey? 'border: 1px solid #C0BFF9;':''" style="position: relative; border-radius:10px;" :key="value.targetKey" :id="'formCard'+value.targetKey" class="fl formDiv commonFormCard" @click="clickForm(value, value.targetKey)">
