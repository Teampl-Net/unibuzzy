        ...
        <fieldset id="neccessaryOptions">
          ...
          <fieldset v-if="$route.path === '/todo' || hasTitleYn" id="postTitle">
            ...
            <textarea
              ...
              style="height:60px; overflow-y:scroll; white-space:wrap; padding:10px 15px 15px;"
              :style="{}"
            />
          </fieldset>
        </fieldset>
  ...
  setup (props) {
    ...
    onMounted(() => {
      ...
      if (props.pContentsData) {
        ...
        if (props.pGetReceiverList && props.pContentsData.refList) {
          refList = props.pGetReceiverList(props.pContentsData.refList)
          if (refList && refList.length > 0) {
            refList.forEach(element => {
              element.actType = 'RF'
            })
            params.refList = refList
          }
        }
        ...
        for (const editingTarget of props.pContentsData.actorList) {
          // params.actorList.length = 0
          if (editingTarget.accessKey === store.getters['D_USER/GE_USER'].userKey) {
            addMeYn = true
          }
    ...
    const priorityList = reactive([
      { value: '00', dispText: 'High' },
      { value: '01', dispText: 'Normal' },
      { value: '02', dispText: 'Low' }
    ])
      ...
      try {
        ...
        if (props.pOptions.model === 'mankik') {
          ...
          if (!params.workToDateStr) {
            ...
            errorText.value = '목표 날짜를 지정해주세요.'
            // } else if (route.path !== '/todo' && (!params.bodyFullStr && !params.attachFileList.length)) {
          } else if (params.stickerList && params.stickerList.length === 0) {
            failPopYn.value = true
            errorText.value = '태그를 선택해주세요.'
          } else {
            ...
            if (hasTitleYn.value && !params.title) {
              if (tempTitle) {
                ...
                failPopYn.value = true
                errorText.value = '제목 또는 내용을 작성해주세요.'
              }
            } else {
              // 제목은 있고, 내용이 없을 경우에는 저장됨
              if (params.title.length > 256) {
                failPopYn.value = true
                errorText.value = '제목은 256자 이내로 작성해주세요.'
              } else {
                params.mLoadingYn = true
                props.pPostContentsFn(params)
              }
          ...
          if (!tempText && !params.attachFileList.length) {
            failPopYn.value = true
            errorText.value = '공유하고자 하는 내용을 작성하거나, 파일을 첨부해주세요.'
          } else {
            if (hasTitleYn.value && !params.title) {
              if (params.bodyFullStr) {
                params.title = titleToBody(doc)
