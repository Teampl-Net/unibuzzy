<template>
    ...
    <pushModal @openLoading="this.loadingYn = true" @closeLoading="this.loadingYn = false" id="pushPopWrap" ref="pushPopWrap"  @closeXPushPop="closeXPushPop" :params="this.pushPopShowYn" v-if="this.pushPopShowYn"/>
    <div @click="showMenuYn = false" v-show="showMenuYn" style="width: 100vw; height: 100vh; background: rgb(0 0 0 / 8%); z-index: 999; top: 0; left: 0; position: fixed;"></div>
...
<style scoped>
  ...
  .TalmenuStyle{height: 100vh; top:0; position: fixed; padding: 0 1rem; box-sizing: border-box; width: 80%; max-width: 500px; background-color: #6768A7; color: #FFFFFF; z-index: 999999; right: 0;}
  .show_view-enter {animation: show_view-dialog-fade-in 0.5s ease;}
  .show_view-leave {animation: show_view-dialog-fade-out 0.5s ease forwards;}
  .show_view-enter-active {animation: show_view-dialog-fade-in 0.5s ease;}
  .show_view-leave-active {animation: show_view-dialog-fade-out 0.5s ease forwards;}
