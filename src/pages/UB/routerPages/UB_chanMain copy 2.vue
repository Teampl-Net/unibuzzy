...
<template>
  <div id="alimWrap" v-if="CHANNEL_DETAIL && CHANNEL_DETAIL.D_CHAN_AUTH" ref="chanScrollWrap" style="overflow: scroll;" :style="settingBackground" class="chanDetailWrap">
    ...
    <div id="summaryWrap" v-if="!mChanInfoPopShowYn" class="summaryWrap mtop-05" style="padding: 0 1rem;">
        ...
        <div class="chanTextBox fl mleft-05;" :class="chanBgBlackYn === true ? 'blackTextBox' : 'whiteTextBox'" style="padding:0.5rem 1rem; width:100%; margin-left: 0.5rem;">
          <div class="fl font16  w100P">
            <p class="font15 textLeft fl">
              <img class="fl img-w20" style="margin-top:2px; margin-right:1rem" src="../../assets/images/channel/channer_4.png" alt="구독자 아이콘">{{ $changeText(CHANNEL_DETAIL.cateItemMtext) }}</p>
          </div>
          <div class="fl font15 w100P " style="box-sizing:border-box; word-break:break-all; ">
            <p class="font14 textLeft fl " style="word-break:break-all">
              <img class="fl img-w20" style="margin-top:2px; margin-right:1rem" src="../../assets/images/channel/channer_3.png" alt="채널 메세지 아이콘"> {{ $changeText(CHANNEL_DETAIL.memoMtext) }}</p>
          </div>
          <div class="fl font15 textLeft w100P " style="box-sizing:border-box; word-break:break-all; ">
            <p class=" commonColor fl fontBold " style="color:#6768a7; white-space: nowrap;"> 개설일</p>
            <p class="fl mleft-1" style="word-break:break-all"> {{ $dayjs(CHANNEL_DETAIL.creDate).format('YYYY-MM-DD') }}</p>
          </div>
          <div class="fl font15 textLeft w100P " style="box-sizing:border-box; word-break:break-all; ">
            <p class=" fl">Followers {{ CHANNEL_DETAIL.followerCount }}</p>
            <p class=" fl mleft-1" style="padding-left: 1rem; border-left: 2px solid #00000050">Posts {{ CHANNEL_DETAIL.totalContentsCount }}</p>
          </div>
        </div>
      </div>
      ...
      <div id="userCardWrap" class="fl w100P" :class="chanBgBlackYn === true ? 'blackTextBox' : 'whiteTextBox'" style="padding:0.5rem 1rem; flex-direction: row; justify-content: space-between;">
        <div class="fl" style="display: flex; align-items: center;">
          ...
          <div class="mleft-05" style="display:flex; flex-direction: column;">
            <p @click="goProfile" class="font16 textLeft">{{ GE_USER.userDispMtext ? $changeText(GE_USER.userDispMtext) : 'Guest' }}</p>
            <div>
        ...
        <div class="fl" style="display: flex; width: 40%; justify-content: space-around; align-items: center;">
          <div style="padding: 3px 10px; border-radius: 10px; border: 1px solid #ccc;"
            v-if="((CHANNEL_DETAIL.userTeamInfo && CHANNEL_DETAIL.userTeamInfo.ownerYn === undefined && CHANNEL_DETAIL.userTeamInfo.memberNameMtext === undefined) || this.$getFollowerType(CHANNEL_DETAIL.D_CHAN_AUTH) === '구독자')">
            <p v-if="(CHANNEL_DETAIL.D_CHAN_AUTH.followYn && !CHANNEL_DETAIL.D_CHAN_AUTH.memberTypeKey && !CHANNEL_DETAIL.D_CHAN_AUTH.ownerYn)"
              class="fl font14 cursorP fontBold commonColor" @click="this.openReqMemPop()">멤버신청</p>
          </div>
          ...
          <div data-clipboard-action="copy" id="copyTextBody" @click="copyText" :data-clipboard-text="CHANNEL_DETAIL.copyTextStr">
            <img class="img-w20" src="../../assets/images/common/icon_share_square.svg" alt="">
          </div>
          <gBtnSmall @click="changeFollowYn" v-if="!CHANNEL_DETAIL.D_CHAN_AUTH.followYn && !GE_USER.unknownYn" class="fl w100P fontBold font14" btnTitle="구독하기" />
        </div>
      </div>

      <div @click="changeFollowYn" id="followerCancelArea" v-if="CHANNEL_DETAIL.D_CHAN_AUTH.followYn && !CHANNEL_DETAIL.D_CHAN_AUTH.ownerYn && CHANNEL_DETAIL.teamKey !== this.$DALIM_TEAM_KEY"
        class="fr" style="padding: 5px 10px; border-radius: 10px; border: 1px solid #ccc;" :style="CHANNEL_DETAIL.D_CHAN_AUTH.followYn ? 'background-color:#DC143C' : 'background-color:#eee'">
        <p class="fl font14 fontBold" :style="CHANNEL_DETAIL.D_CHAN_AUTH.followYn ? 'color:white' : ''">구독취소</p>
      </div>
    </div>

    <div class="channelItemBox" ref="channelItemBoxPushListDivCompo" id="channelItemBox" style="margin-top: 250px; background: rgb(220, 221, 235); padding-top: 0; overflow: hidden;">
      <pushList :pUnknownYn="mUnknownYn" @openImgPop="openImgPop" @goScroll="mChanMainScrollWrap.style.overflow = 'scroll'" :initData="this.propParams.initData.contentsList" @cMemoEditYn="changeMemoEditYn"
        :targetContents="{ targetContentsKey: propParams.targetContentsKey, jobkindId: propParams.jobkindId }" :chanAlimYn="true" :pChannelDetail="this.CHANNEL_DETAIL" :chanAlimTargetType="this.propParams.targetType"
        ...
        @targetContentScrollMove='targetContentScrollMove' @openLoading="this.$emit('openLoading')"
        @closeLoading="this.$emit('closeLoading')" @openUserProfile='openItem' @changeMainTab='changeMainTab'
        isOpen='chanAlim' @memoEdit='memoEdit' />
    </div>

    <div v-if="this.mChanInfoPopShowYn">
      <chanDetailComp ref="chanDetailRef" @openLoading="this.$emit('openLoading')"
        @closeLoading="this.$emit('closeLoading')" @closeXPop="this.closeDetailPop"
        @changeshowProfileYn='changeshowProfileYn' :parentshowProfileYn="CHANNEL_DETAIL.D_CHAN_AUTH.showProfileYn"
        :adminYn="CHANNEL_DETAIL.D_CHAN_AUTH.adminYn" :alimSubPopYn="alimListToDetail"
        @pageReload="this.$emit('pageReload', true)" @openPop="openPushDetailPop" @closeDetailPop="this.closeDetailPop"
        @changeFollowYn="changeFollowYn" :chanDetail="this.CHANNEL_DETAIL" style="background-color: #fff;">
      </chanDetailComp>
    </div>
    <img id='writeBtn' src="../../assets/images/button/Icon_WriteAlimBtn.png" @click="openWritePushPop" alt="알림 작성 버튼"
      style="position: absolute; bottom: 2rem; right: 10%; z-index:9; cursor: pointer;" class="img-78 img-w66">
    <gConfirmPop :confirmText='mErrorPopBodyStr' :confirmType='mErrorPopBtnType' @no='mErrorPopShowYn = false'
      v-if="mErrorPopShowYn" @ok="confirmOk" />
    <div v-if="writeBottSheetYn" @click="writeBottSheetYn = false"
      style="width: 100%; height: 100%; position: absolute; z-index: 10; left: 0; top: 0; background: #00000030;"></div>
    <transition name="showUp">
      <writeBottSheet v-if="writeBottSheetYn === true" @openPop='openWriteContentsPop'
        @closePop='writeBottSheetYn = false' :propTeamKey='CHANNEL_DETAIL.teamKey' @openMember='openChannelMsgPop()' />
    </transition>
  </div>
  <div v-if="writeContentsYn === true"
    style="position: absolute; top:0; left:0; z-index:10; background:#00000050; width: 100vw; min-height: 100vh;"
    class=""></div>
  <writeContents v-if="writeContentsYn === true" ref="chanAlimListWritePushRefs"
    :contentType="mPushListMainTab === 'P' ? 'ALIM' : 'BOAR'" @closeXPop='closeWritePushPop' :params="mWriteAlimData"
...
import pushList from './Tal_pushList.vue'
import welcomePopUp from '../../components/pageComponents/channel/Tal_chanFollowInfo.vue'
...
export default {
  ...
  methods: {
    ...
    targetContentScrollMove (wich, jobkindId) {
      ...
      if (wich || this.propParams.targetContentsKey) {
    ...
    async readyFunction () {
      ...
      if (directUrl) {
        ...
        if (this.propParams.initData) {
          if (this.propParams.initData.team.D_CHAN_AUTH && this.propParams.initData.team.D_CHAN_AUTH.followYn) {
            this.$emit('followYn')
          }
          if (this.CHANNEL_DETAIL) {
            console.log(this.propParams.initData.team.copyTextStr === undefined)
            if ((this.propParams.initData.team.copyTextStr === undefined && this.CHANNEL_DETAIL.copyTextStr === undefined) && !this.mMakeDeepLinkIng) {
    ...
    async changeRecvAlimYn () {
      ...
      this.CHANNEL_DETAIL.copyTextStr = this.propParams.initData.team.copyTextStr
      this.$store.dispatch('D_CHANNEL/AC_ADD_CHANNEL', [this.CHANNEL_DETAIL])
      setTimeout(() => {
    ...
    updateScroll () {
      ...
      if (this.mChanMainScrollDirection === 'down' && this.mChanMainScrollPosition > 150) {
        blockBox.style.height = 50 + 'px'
        if (this.mChanMainScrollPosition > 190) this.mChanMainScrollWrap.style.overflow = 'hidden'
  ...
  computed: {
    CHANNEL_DETAIL () {
      if (!this.propParams && !this.mDirectTeamKey) return {}
      let teamKey
      if (this.propParams) {
