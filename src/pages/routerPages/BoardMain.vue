...
<template>
  <div>
    ...
    <div
      v-if="mPinPostPopShowYn"
      @click="closePinPostPop = false"
      class="w100P h100P popBg"
    ></div>
    <pinPostPop
      v-if="mPinPostPopShowYn"
      :pUpdateTopview="updateTopview"
      :pTVList="
        this.CAB_DETAIL.topviewList ? this.CAB_DETAIL.topviewList.content : []
      "
      :pChanDetail="CHANNEL_DETAIL"
      :pClosePop="closePinPostPop"
      :pBoardDetail="CAB_DETAIL"
    />
    <div
      id="boardWrap"
      v-if="CAB_DETAIL"
      :style="
        CAB_DETAIL.picBgPath
          ? 'background: ' +
            CAB_DETAIL.picBgPath +
            ';' +
            'padding-top: ' +
    ...
    >
      <p
        class="font20 fontBold textOverdot cabinetName"
        :style="
          CAB_DETAIL.cabinetNameMtext.length > 15
            ? 'font-size:18px !important;'
            : ''
        "
      >
        {{ this.$changeText(CAB_DETAIL.cabinetNameMtext) }}
      </p>
      <div id="summaryHeader" class="summaryHeader">
        <div id="boardInfoSummary" class="mt-header boardWhiteBox">
          <div
            ...
            id="chanInfoSummary"
            ref="chanImg"
            class="boardCard chanInfoSummary"
          >
            {{ value }}
            <div
              class="chanImgRound"
            ...
            <div class="chanTextBox fl mleft-05 w100P">
              <div class="fl font16 w100P">
                <div class="width50px">
                  <p
                    class="font15 commonColor textLeft fl mleft-05 categoryTitle"
                  >
                    Channel
                  </p>
                </div>
                <p class="font14 textLeft fl mleft-1 commonBlack">
                  {{ this.$changeText(CHANNEL_DETAIL.nameMtext) }}
                </p>
              </div>
              <div class="fl font16 w100P">
                <div class="width50px">
                ...
                <p class="font14 textLeft fl mleft-1 commonBlack">
                  {{ this.$changeDateFormat(CAB_DETAIL.creDate) }}
                </p>
              </div>
            </div>
            <gBtnSmall
              ...
              v-if="
                (CHANNEL_DETAIL &&
                  (this.CHANNEL_DETAIL.D_CHAN_AUTH.mngAlimYn === 1 ||
                    this.CHANNEL_DETAIL.D_CHAN_AUTH.ownerYn ||
                    this.CHANNEL_DETAIL.D_CHAN_AUTH.mngMemberYn === 1)) ||
                this.CHANNEL_DETAIL.D_CHAN_AUTH.mngTeamYn === 1
              "
              btnTitle="Pin a Post"
              @click="openPinPostPop"
            />
          </div>
          <div class="fl w100P boardCard mtop-05 totalCountBox">
            <p class="cBlack fl font15 w100P">
              Shared members: {{ CAB_DETAIL.mShareItemCnt }}
            </p>
            <p class="cBlack fl font15 w100P">
              Total Post: {{ this.totalElements ? this.totalElements : 0 }}
            </p>
          </div>
        </div>
        <div
          v-if="CHANNEL_DETAIL"
          id="boardInfoSummary2"
          class="summaryHeader2"
        >
          <span class="font20 fontBold">{{
            this.$changeText(CAB_DETAIL.cabinetNameMtext)
          }}</span>
          <span class="font13 mbottom-05 fl">{{
            this.$changeText(CHANNEL_DETAIL.nameMtext)
          }}</span>
        </div>
        <div
          ...
          v-if="
            this.CAB_DETAIL &&
            this.CAB_DETAIL.topviewList &&
            this.CAB_DETAIL.topviewList.content &&
            this.CAB_DETAIL.topviewList.content.length > 0
          "
          class="w100P flexAlignCenter flexColumn"
        >
          <div class="textLeft fontBold pinTitle">
            <img
              src="@/assets/images/board/icon_paperPin.svg"
              alt="pin image"
            />
            Pin
          </div>
          ...
          >
            <gTopViewItem
              ...
              :contentsEle="cont"
              @openPop="openPop"
              v-for="(cont, index) in this.CAB_DETAIL.topviewList.content"
              :key="index"
            />
          </div>
        </div>
      ...
      <div class="boardItemBox" id="boardItemBox">
        <div
          id="boardListWrap"
          class="boardContentsWrap"
          ref="boardListWrapCompo"
        >
          <transition name="showModal">
            <findContentsList
              :pOnlyMineYn="mOnlyMineYn"
              :tpGroupCode="
                CAB_DETAIL.workStatYn === 1 || CAB_DETAIL.workStatYn === true
                  ? 'C_STAT'
                  : null
              "
              :contentsListTargetType="'boardMain'"
          ...
          <div
            id="commonBoardListHeader"
            ref="boardListHeader"
            class="boardListHeader"
            :class="
              this.scrolledYn
          ...
          <div
            :style="calcBoardPaddingTop"
            style="
              padding-top: calc(60px + var(--paddingTopLength));
              height: calc(100%);
              padding-bottom: 40px;
              min-height: 500px;
            "
            class="commonBoardListWrap"
          ...
          >
            ...
            <gUBContentsBox
              ...
              :contentsEle="cont"
              @openPop="openPop"
              v-for="cont in this.BOARD_CONT_LIST"
            ...
            ></myObserver>
            <gEmpty
              v-if="emptyYn && BOARD_CONT_LIST.length === 0"
      ...
      <div
        :class="
          this.scrolledYn || !this.reloadShowYn
            ? 'reload--unpinned'
            : 'reload--pinned'
        "
        v-on="handleScroll"
      ...
      <img
        ...
        v-if="
          CAB_DETAIL.cabinetKey === 13909 ||
          CAB_DETAIL.cabinetKey === 13905 ||
          (CHANNEL_DETAIL &&
            CAB_DETAIL.shareAuth &&
            this.CAB_DETAIL.shareAuth.W === true &&
            CHANNEL_DETAIL.D_CHAN_AUTH.followYn)
        "
        @click="openWriteBoard"
        alt="button for write contents"
        class="img-78 writeBtn"
      />
    </div>
    <gConfirmPop
      :confirmText="errorBoxText"
      :confirmType="confirmType ? 'two' : 'timeout'"
      @no=";(errorBoxYn = false), (reportYn = false)"
      @ok="confirmOk"
      v-if="errorBoxYn"
    />
    <div v-if="boardWriteYn" class="writePopBg"></div>
    <writeContents
      class="writeContentsPop"
      v-if="boardWriteYn"
      ref="chanAlimListWritePushRefs"
      @successWrite="successWriteBoard"
      @successSave="getContentsList"
      :contentType="currentPushListMainTab === 'P' ? 'ALIM' : 'BOAR'"
      @closeXPop="closeWriteBoardPop"
      :propData="boardWriteData"
      :params="boardWriteData"
      @openPop="openItem"
      :changeMainTab="changeMainTab"
      @toAlimFromBoard="toAlimFromBoard"
      :propParams="boardWriteData"
    />
    <div
      v-if="memoShowYn === true"
      class="boardMainMemoBoxBackground"
      @click="memoPopNo()"
    ></div>
    <transition name="showMemoPop">
      <gMemoPop
        ...
        :mememo="mememoValue"
        @mememoCancel="mememoCancel"
        :writeMemoTempData="tempMemoData"
      />
    </transition>
    <gReport
      v-if="reportYn"
      @closePop="reportYn = false"
    ...
    />
    <gSelectBoardPop
      :type="this.selectBoardType"
      @closeXPop="closeSelectBoardPop"
      v-if="selectBoardPopShowYn"
      :boardDetail="boardDetailValue"
      :boardValue="detailVal"
    />
  </div>
</template>
<script>
    ...
    if (!this.mPropParams || this.mPropParams.targetType !== 'boardMain') {
      ...
      this.getCabinetDetail().then(() => {
        ...
        this_.getContentsList().then((response) => {
          ...
          var newArr = [...this_.BOARD_CONT_LIST, ...response.content]
          this_.mCabContentsList = this.replaceArr(newArr)
          this.$emit(
            'changePageHeader',
            this.$changeText(this.CAB_DETAIL.cabinetNameMtext)
          )
          this.$emit('clearInfo', {
            detail: this.CAB_DETAIL,
            targetType: 'boardMain'
          })
          this_.readyFunction()
        })
      })
      ...
      var propBoardInitData = JSON.parse(
        JSON.stringify(this.mPropParams.initData)
      )
      this.cabinetDetail = propBoardInitData.cabinet
      this.cabinetDetail.shareAuth = this.$checkUserAuth(
        propBoardInitData.cabinet.mShareItemList
      )
      ...
      if (propBoardInitData.contentsListPage) {
        ...
        if (
          propBoardInitData.contentsListPage.totalElements <
          propBoardInitData.contentsListPage.pageable.offset +
            propBoardInitData.contentsListPage.pageable.pageSize
        ) {
          this.endListYn = true
        }
      } else {
  ...
  methods: {
    ...
    updateTopview(type, board) {
      if (type === 'add') {
        ...
        const index = this.cabinetDetail.topviewList.content.findIndex(
          (item) => {
    ...
    async getContentsMemoList(key, pageSize, offsetInt) {
      if (
        this.axiosQueue.findIndex((item) => item === 'getContentsMemoList') !==
        -1
      ) {
        return
      }
      ...
      if (offsetInt !== undefined && offsetInt !== null && offsetInt !== '') {
        memo.offsetInt = offsetInt
      } else {
    ...
    async saveMemo(inSaveMemoObj) {
      ...
      if (this.axiosQueue.findIndex((item) => item === 'saveMemo') !== -1) {
        return
      }
      ...
      if (this.mememoValue !== undefined && this.mememoValue !== null) {
      ...
      memo.creUserName = this.$changeText(
        this.GE_USER.userDispMtext || this.GE_USER.userNameMtext
      )
      memo.userName = this.$changeText(
        this.GE_USER.userDispMtext || this.GE_USER.userNameMtext
      )
      try {
        var result = await this.$commonAxiosFunction({
        ...
        if (result.data.result === true || result.data.result === 'true') {
          ...
          idx = this.BOARD_CONT_LIST.findIndex(
            (i) => i.contentsKey === this.currentContentsKey
          )
          if (idx !== -1) {
          ...
          if (
            memoLength !== undefined &&
            memoLength !== null &&
            memoLength !== ''
          ) {
            var response = await this.getContentsMemoList(
              this.currentContentsKey,
              memoLength + 1,
              0
            )
            if (!cont.D_MEMO_LIST) cont.D_MEMO_LIST = []
            var newArr = [...response, ...cont.D_MEMO_LIST]
      ...
      try {
        if (this.mobileYn) {
          ...
          await this.$downloadFile(
            this.selectImgObject.fileKey,
            this.selectImgObject.path
          )
        }
    ...
    async yesLoadMore(contentKey) {
      ...
      var response = await this.getContentsMemoList(
        contentKey,
        cont.D_MEMO_LIST.length + 5,
        0
      )
      var newArr = [...cont.D_MEMO_LIST, ...response]
    ...
    writeMemo(param) {
      ...
      var idx
      idx = this.BOARD_CONT_LIST.findIndex(
        (i) => i.contentsKey === param.contentsKey
      )
      if (idx !== -1) {
    ...
    makeNewContents(data) {
      ...
      if (data.attachMfilekey) {
        param.attachMfilekey = data.attachMfilekey
      }
    ...
    delContents(cont) {
      ...
      if (cont.jobkindId === 'BOAR') {
        idx = this.mCabContentsList.findIndex(
    ...
    async confirmOk() {
      ...
      if (this.currentConfirmType === 'deleteBoar') {
        ...
        this.deleteMemo({
          memoKey: this.tempData.memoKey,
          contentsKey: this.tempData.targetKey,
          parentMemoKey: this.tempData.parentMemoKey
        })
      }
    },
    async deleteMemo(param) {
      if (this.axiosQueue.findIndex((item) => item === 'deleteMemo') !== -1) {
        return
      }
      var memo = {}
      ...
      if (result.data.result === true) {
        ...
        idx = this.BOARD_CONT_LIST.findIndex(
          (i) => i.contentsKey === this.tempData.targetKey
        )
        if (idx !== -1) cont = this.BOARD_CONT_LIST[idx]

        index = cont.D_MEMO_LIST.findIndex(
          (item) => item.memoKey === param.memoKey
        )
        var cmemoListIdx
        if (param.parentMemoKey) {
          for (let i = 0; i < cont.D_MEMO_LIST.length; i++) {
            if (cont.D_MEMO_LIST[i].cmemoList.length > 0) {
              index = cont.D_MEMO_LIST[i].cmemoList.findIndex(
                (i) => i.memoKey === param.memoKey
              )
              if (index !== -1) {
                cmemoListIdx = i
                break
    ...
    openUpdateContentsPop() {
      ...
      if (this.tempData.attachMfilekey) {
        param.attachMfilekey = this.tempData.attachMfilekey
      }
    ...
    async recvNoti(e) {
      ...
      try {
        ...
        if (
          message.actType === 'WR' &&
          JSON.parse(message).targetKey === this.mPropParams.targetKey
        ) {
          this.refresh()
        }
      } catch (err) {
    ...
    handleScroll() {
      ...
      this.firstContOffsetY =
        this.getAbsoluteTop(element) - this.getAbsoluteTop(parentElement)
      ...
      if (
        time / 1000 > 1 &&
        this.$diffInt(this.listBox.scrollTop, this.scrollPosition) > 150
      ) {
    ...
    openWriteBoard() {
      ...
      if (
        !this.GE_USER.certiDate &&
        (this.CAB_DETAIL.blindYn === 1 || this.CAB_DETAIL.blindYn === true)
      ) {
        // 익명게시판일 떄
      ...
      params.bodyFullStr = ''
      params.cabinetNameMtext = this.$changeText(
        this.CAB_DETAIL.cabinetNameMtext
      )
    ...
    updateScroll() {
      ...
      if (this.scrollDirection === 'down' && this.scrollPosition > offset) {
        ...
        document
          .getElementById('boardItemBox')
          .classList.add('boardItemBoxHeight')
        ...
        document
          .getElementById('boardInfoSummary')
          .classList.remove('displayNIm')
        if (document.getElementById('topViewWrap')) {
          document.getElementById('topViewWrap').classList.remove('displayNIm')
        }
        document
          .getElementById('boardItemBox')
          .classList.remove('boardItemBoxHeight')
    ...
    goDetail(value) {
      if (
        this.CAB_DETAIL.shareAuth.V === false &&
        value.creUserKey !== this.GE_USER.userKey
      ) {
    ...
    async changeTab(tabName) {
      ...
      if (
        !resultList ||
        (resultList.content && resultList.content.length === 0)
      ) {
        this.mCabContentsList = []
      } else {
    ...
    updateStoreBoardList(uniqueArr) {
      ...
      var index = cabinetList.findIndex(
        (item) => item.cabinetKey === this.CAB_DETAIL.cabinetKey
      )
    ...
    async loadMore(pageSize) {
      if (this.endListYn === false) {
        if (
          this.mCabContentsList &&
          !this.CAB_DETAIL.totalContentsCount > this.mCabContentsList.length
        ) {
          return
        }
        ...
        const newArr = [...this.BOARD_CONT_LIST, ...resultList.content]
  ...
  computed: {
    ...
    BOARD_CONT_LIST() {
      ...
      for (let i = 0; i < this.mCabContentsList.length; i++) {
        vPoolChanIdx = this.GE_MAIN_CHAN_LIST.findIndex(
          (item) => item.teamKey === this.mCabContentsList[i].creTeamKey
        )
        ...
        vPoolContentIdx = vPoolContList.findIndex(
          (item) => item.mccKey === this.mCabContentsList[i].mccKey
        )
  ...
  watch: {
    ...
    GE_NEW_MEMO_LIST: {
      async handler(value, old) {
        ...
        var content = null
        var index = this.mCabContentsList.findIndex(
        ...
        var idx1 = this.mCabContentsList.findIndex(
    ...
    GE_NEW_CONT_LIST: {
      handler(value, old) {
        ...
        if (value[0].cabinetKey === this.CAB_DETAIL.cabinetKey) {
          newArr = [value[0], ...this.mCabContentsList]
