...
<template>
  <div class="popBg" v-if="mBoardPopShowYn || mComingSoon" @click.stop></div>
  <boardListPop v-if="mBoardPopShowYn" :pChanDetail="CHANNEL_DETAIL" :pClosePop="closeBoardPop"/>
  <commpingSoonPop v-if="mComingSoon" :pClosePop="closeComingSoon" :pClickedInfo="mClickedInfo"/>
  <!-- <friendFinderPop v-if="mFriendFinderShowYn" :pClosePop="mCloseFFPop" /> -->
...
import commpingSoonPop from '@/components/popup/D_ComingSoonPop.vue'
  ...
  methods: {
    ...
    moveToSelectInfo (area) {
      ...
      if (area === 0) {
        ...
        // this.$router.push('/friendfind')
        this.openComingSoon()
        this.mClickedInfo = 'Friends'
      } else if (area === 6) {
        this.openComingSoon()
        this.mClickedInfo = 'Nearby'
      }
      // }
    }
  },
