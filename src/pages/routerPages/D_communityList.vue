<i18n>
  ...
  <template>
      <div class="h100P w100P" style="overflow: hidden; background-color:#fff;" :style="`padding-top: ${this.$STATUS_HEIGHT + 50}px`" >
          <div style="width: 100%; height: 100%;  float: left; background: #fff; position: relative;">
              ...
              <div v-if="(GE_DISP_TEAM_LIST && GE_DISP_TEAM_LIST.length === 0) && mEndListYn === false" style="margin-top: 80px; width: 100%; min-height: 100%;">
              ...
              <div id="chanListWrap" ref="chanListWrap" :style="calcPaddingTop" style="padding-top: calc(25px + var(--paddingTopLength)); overflow: hidden scroll; height: 100%; width: 100%; " @mousedown="testTwo" @mouseup="testTr">
                  <template v-if="mCurrentTabName === $t('COMMON_NAME_CHANNEL')" >
                      <!-- <div v-if="(GE_DISP_TEAM_LIST && GE_DISP_TEAM_LIST.length === 0) && mEndListYn === false" style="margin-top: 80px; width: 100%; min-height: 100%;">
                          <chanSkeleton  v-for="(value) in 10" :key="value"/>
                      </div> -->
                      <gEmpty v-if="GE_DISP_TEAM_LIST && GE_DISP_TEAM_LIST.length === 0 && mEmptyYn" :tabName="mCurrentTabName" contentName="채널" style="margin-top:50px;" />
                      <template v-for="(chanEle, index) in mContentsInfo.team" :key="index">
                  ...
                  <template v-else>
                      <gEmpty :tabName="mCurrentTabName" contentName="게시판" v-if="mEmptyYn && this.GE_DISP_TEAM_LIST.length === 0" style="margin-top:50px;" />
                      <template v-for="(boardElement, index) in mContentsInfo.cabinet" :key="index">
                          <boardCard class="moveBox chanRow" @openPop="openPop" :boardElement="boardElement" />
                      </template>
                  </template>
              </div>
...
import boardCard from '@/components/list/D_boardCard.vue'
...
export default {
  ...
  data () {
    return {
      ...
      mEndListYn: false,
      mViewTab: 'user',
      mActiveTabList: [{ display: this.$t('COMMON_NAME_CHANNEL'), name: 'channel' }, { display: this.$t('COMMON_NAME_BOARD'), name: 'board' }],
      mChanFindPopShowYn: false,
      mResultSearchKeyList: '',
      myChanListPopYn: false,
      mScrollCheckSec: 0,
      mListShowYn: true,
      mCurrentTabName: this.$t('COMMON_NAME_CHANNEL'),
      mEmptyYn: true,
      mLoadingYn: false,
      mAxiosQueue: [],
      mSearchCateKey: 0,
  ...
  created () {
    console.log('GE_DISP_CHAN_LIST', this.GE_DISP_TEAM_LIST)
    // this.getContentsList()
    this.$emit('changePageHeader', this.$t('COMMON_NAME_COMMUNITY'))
    if (this.propData) {
    ...
    if (this.initData) {
      this.mChannelList = this.initData.chanList
      if (!this.mChannelList) {
        this.getContentsList()
      } else {
        console.log('mChannelList', this.mChannelList)
      }
    }
  ...
  methods: {
    async getContentsList () {
      let params = []
      if (this.mCurrentTabName === this.$t('COMMON_NAME_CHANNEL')) {
        params = {
          parentTeamKey: this.GE_USER.myTeamKey,
          communityType: 'C'
        }
      } else if (this.mCurrentTabName === this.$t('COMMON_NAME_BOARD')) {
        params = {
          parentTeamKey: this.GE_USER.myTeamKey,
          communityType: 'B'
        }
      ...
      if (result) {
        console.log('mContentsInfo', result)
        if (this.mNowChannel) {
          this.endListSetFunc(this.mNowChannel)
        }
        this.mEndListYn = true
        this.mEmptyYn = false
        this.mContentsInfo = result.data
      }
    },
    goSavePhonePop () {
    ...
    introChanPageTab () {
      if (this.mViewTab === 'channel') {
        this.mCurrentTabName = this.$t('COMMON_NAME_CHANNEL')
      } else if (this.mViewTab === 'board') {
        this.mCurrentTabName = this.$t('COMMON_NAME_BOARD')
      }
    },
    ...
    endListSetFunc (resultList) {
      ...
      if (!resultList.totalElements || !resultList.pageable || (resultList.totalElements < (resultList.pageable.offset + resultList.pageable.pageSize))) {
        this.mEndListYn = true
    ...
    clickCreateChannel () {
      ...
      if (this.GE_USER.certiDate) {
        ...
        tempParam.parentTeamKey = this.GE_USER.myTeamKey
        tempParam.callBackFn = this.getContentsList
        this.$emit('openPop', tempParam)
      } else {
        this.gCertiPopShowYn = true
      }
    ...
    async requestSearchList (paramMap) {
      ...
      this.mChannelList = resultList.content
      this.mViewTab = 'channel'
      this.$refs.activeBar.switchtab(1)// 전체
      this.$refs.activeBar.selectTab('channel')// 전체
      this.mChanFindPopShowYn = false
    },
  ...
  watch: {
    ...
    GE_USER: {
      ...
      handler (val, old) {
        if (val.unknownYn) {
          this.mActiveTabList = [{ display: this.$t('COMMON_NAME_CHANNEL'), name: 'channel' }]
        } else {
          this.mActiveTabList = [{ display: this.$t('COMMON_NAME_CHANNEL'), name: 'channel' }, { display: this.$t('COMMON_NAME_BOARD'), name: 'board' }]
        }
      }
    },
    GE_UPDATE_CHAN_LIST: {
