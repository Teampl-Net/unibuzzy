<template>
  <div class="w100P h100P textCenter developerWrap">
    ...
    <div class="w100P fl p-20 patchArea">
      ...
      <p class="textLeft mRight-30 fl commonNormalText">
        마지막 패치 {{ mPatchTime }}
      </p>
      <input
        class="fl patchInput"
      ...
      <gBtnSmall
        class="mtop-1"
        :btnTitle="mBtnList[1].btnText"
        @click="mBtnList[1].onClickFn"
      />
    </div>
  </div>
</template>
...
export default {
  async created() {
    if (localStorage.getItem('patchTime')) {
      this.mPatchTime = localStorage.getItem('patchTime')
    }
  },
  data() {
    return {
      ...
      mBtnList: [
        {
          ...
          btnText: '등록하기',
          pointYn: true,
          onClickFn: this.saveGoji
        },
        {
          btnType: 'noBorder',
          btnText: '실행',
          pointYn: true,
          onClickFn: this.savePatchTime
        }
      ]
    }
  },
