    ...
    <div class="w100P mainTownArea">
      ...
      <template v-if="!mLoadingYn">
        <template v-for="area in mBdAreaList" :key="area.bdAreaKey">
          <div
            v-if="
              village.areaList[area.priority].w !== 0 &&
              village.areaList[area.priority].h !== 0
            "
            class="flexCenter areaDiv"
            :class="{ clicked: village.areaList[area.priority].clickedYn }"
            :style="{
              width: village.areaList[area.priority].w + 'px',
              height: village.areaList[area.priority].h + 'px',
              top: village.areaList[area.priority].top + 'px',
              left: village.areaList[area.priority].left + 'px'
            }"
          >
            <img
              :src="village.areaList[area.priority].maskedImageUrl"
              :style="village.areaList[area.priority].maskedImageStyle"
            />
            <div
              v-if="area.bdAreaNameMtext"
              class="fontBold"
              :style="{
                'margin-top':
                  area.priority !== 0 && area.priority !== 1
                    ? 15 + village.areaList[area.priority].h / 1.5 + 'px'
                    : ''
              }"
            >
              <p class="textCenter fontBold font16">
                {{ area.bdAreaNameMtext }}
              </p>
            </div>
          </div>
          <template v-for="(bd, index) in area.bdList" :key="bd.targetKey">
            <div
              ref="bdRef"
              :id="`area${area.bdAreaKey}bd${bd.bdKey}pri${bd.priority}`"
              v-if="
                village.areaList[area.priority].buildingList[index] &&
                village.areaList[area.priority].buildingList[index].w !== 0 &&
                village.areaList[area.priority].buildingList[index].h !== 0
              "
              class="bdDiv"
              :class="{
                clicked:
                  village.areaList[area.priority].buildingList[index].clickedYn
              }"
              :style="[
                `z-index: ${1000 - bd.priority};`,
                village.areaList[area.priority].buildingList[index]
                  .maskedImageStyle,
                {
                  top:
                    village.areaList[area.priority].buildingList[index].top +
                    'px',
                  left:
                    village.areaList[area.priority].buildingList[index].left +
                    'px'
                }
              ]"
            >
              <div
                v-if="area.priority === 0"
                class="banner flexCenter"
                :style="{
                  left:
                    -(
                      125 -
                      village.areaList[area.priority].buildingList[index].w / 2
                    ) + 'px'
                }"
              >
                <img src="@/assets/images/main/banner2.png" class="w100P" />
                <!-- 여기 -->
                <div
                  v-html="$changeText(bd.nameMtext)"
                  class="w100P font16 fontBold"
                ></div>
              </div>
              <img
                :src="
              ...
              <span
                v-if="
                  !(area.priority === 0 && index === 0) &&
                  village.areaList[area.priority].buildingList[index]
                    .maskedImageUrl
                "
                class="fontBold font12 bdName"
                :style="[
                  {
                    'background-color':
                      index === 0
                  ...
                  },
                  {
                    left:
                      -40 +
                      village.areaList[area.priority].buildingList[index].w /
                        2 +
                      'px'
                  },
                  {
                    top:
                      village.areaList[area.priority].buildingList[index].h +
                      (Number(bd.priority) / 2) * 10 +
                      'px'
                  }
                ]"
                >{{
                  $changeText(bd.nameMtext) || $changeText(bd.cabinetNameMtext)
                }}</span
              >
            </div>
          </template>
