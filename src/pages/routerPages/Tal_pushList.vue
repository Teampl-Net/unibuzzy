    ...
    <div style="width: 100%; height: 100%; padding-top: 0; position: relative; overflow: hidden; float: left;" >
      ...
      <!-- <div id="pushListWrap" class="pushListWrapWrap" ref="pushListWrapWrapCompo" :style="calcPaddingTop" style="position: relative; float: left; width: 100%; padding-top: calc(125px + var(--paddingTopLength)); overflow: hidden scroll; height: calc(100%); "> -->
        ...
        <div id="pushListWrap" class="pushListWrapWrap " ref="pushListWrapWrapCompo" :style="'padding: 0 1rem ; padding-top:' + this.paddingTop + 'px;'" style="position: relative; margin-top: 1rem; float: left; width: 100%; overflow: hidden scroll; height: calc(100%); padding-bottom: 40px; ">
          ...
          <template  v-for="(cont) in this.GE_DISP_BOAR_LIST" :key="cont.contentsKey">
          ...
          <template  v-for="(cont) in this.GE_DISP_ALIM_LIST" :key="cont.contentsKey">
          ...
          <template v-for="(cont, index) in this.GE_DISP_ALL_LIST" :key="cont.contentsKey">
            ...
            <myObserver v-if="index === this.GE_DISP_ALL_LIST.length - 15" @triggerIntersected="loadMore" id="observer" class="fl w-100P" style=""></myObserver>
          </template>
          ...
          <template  v-for="(cont, index) in this.GE_FILE_LIST" :key="cont.contentsKey">
  ...
  methods: {
    ...
    async getPushContentsList (pageSize, offsetInput, loadingYn) {
      if (this.axiosQueue.findIndex((item) => item === 'getPushContentsList') === -1) {
        ...
        // eslint-disable-next-line no-debugger
        debugger
        if (this.pMyCabinetKeyListStr || (this.chanAlimYn && this.GE_CHANNEL_DETAIL && this.GE_CHANNEL_DETAIL.cabinetKeyListStr)) {
          if (this.pMyCabinetKeyListStr) {
            param.cabinetKeyListStr = this.pMyCabinetKeyListStr
          } else {
            param.cabinetKeyListStr = this.GE_CHANNEL_DETAIL.cabinetKeyListStr
          }
        }
