  ...
  mutations: {
    ...
    MU_MAIN_CHAN_LIST: (state, payload) => {
      for (var i = 0; i < payload.length; i++) {
        var team = payload[i]
        if (team.get('ELEMENTS')) {
          var teamElements = team.get('ELEMENTS')
          if (!teamElements.get('alimList')) { teamElements.set('alimList', new Map()) }
          if (!teamElements.get('commonList')) { teamElements.set('commonList', new Map()) }
          if (!teamElements.get('boardList')) { teamElements.set('boardList', new Map()) }
          if (!teamElements.get('cabinetList')) { teamElements.set('cabinetList', new Map()) }
          if (!teamElements.get('showProfileUserList')) { teamElements.set('showProfileUserList', new Map()) }
          if (!teamElements.get('managerList')) { teamElements.set('managerList', new Map()) }
        } else {
          team.set('ELEMENTS', new Map ([['alimList', new Map()], ['boardList', new Map()], ['cabinetList', new Map()], ['managerList', new Map()], ['showProfileUserList', new Map()] ])
        }
        team.set('teamTypeText',commonMethods.teamTypeString(team.teamType))
        ...
        team.set('D_CHAN_AUTH', D_CHAN_AUTH)
        state.chanList.set(team.teamKey, team)
      }
    ...
    MU_ADD_CHANNEL: (state,                                                                                                                                                                                                                                                                                                                                                                                                                                 ) => {
      var index
      for (var i = 0; i < payload.length; i++) {
        // input값
        var team = payload[i]
        // 채널 찾기
        var findTeam = state.chanList.put(team.teamKey)
        if (findTeam) {
