...
const D_CHANNEL = {
  ...
  mutations: {
    ...
    MU_ADD_MEMO: (state, payload) => {
      ...
      } else if (payload.jobkindId === 'MEMO') {
        dataList = chan.ELEMENTS.memoList
        index3 = chan.ELEMENTS.memoList.findIndex((item) => item.contentsKey === Number(payload.targetKey))
      } else {
      ...
      } else if (dataList[index3].jobkindId === 'MEMO') {
        if (newIdx === -1) {
          state.chanList[chanIndex].ELEMENTS.memoList[index3].D_MEMO_LIST.unshift(payload)
          state.addMemoList.unshift(payload)
        } else {
          state.chanList[chanIndex].ELEMENTS.memoList[index3].D_MEMO_LIST[newIdx] = payload
          state.addMemoList.unshift(payload)
        }
      } else {
        if (newIdx === -1) {
    ...
    MU_MAIN_CHAN_LIST: (state, payload) => {
      for (var i = 0; i < payload.length; i++) {
        ...
        if (team.ELEMENTS) {
          ...
          if (!team.ELEMENTS.memoList) { team.ELEMENTS.memoList = [] }
        ...
        // var title = '[하이브릭]' + commonMethods.changeText(team.nameMtext)
      ...
      for (var i = 0; i < payload.length; i++) {
        ...
        if (index === -1) {
          if (team.ELEMENTS) {
            ...
            team.ELEMENTS = { alimList: [], boardList: [], todoList: [], memoList: [], cabinetList: [], commonList: { type: 'ALIM', list: [] }, managerList: [], showProfileUserList: [] }
          }
          team.teamTypeText = commonMethods.teamTypeString(team.teamType)
          // eslint-disable-next-line no-new-object
    ...
    MU_REPLACE_CONTENTS: (state, payload) => {
      ...
      if (idx1 !== -1) {
        ...
        for (var i = 0; i < payload.length; i++) {
          ...
          } else if (payload[i].jobkindId === 'MEMO') {
            idx2 = chanDetail.ELEMENTS.memoList.findIndex((item) => item.contentsKey === payload[i].contentsKey)
          } else {
          ...
          if (idx2 === -1) {
            if (payload[i].jobkindId === 'BOAR') {
              ...
              chanList[idx1].ELEMENTS.memoList.push(payload[i])
              idx2 = chanDetail.ELEMENTS.memoList.findIndex((item) => item.contentsKey === payload[i].contentsKey)
            } else {
          ...
          if (payload[i].jobkindId === 'BOAR') {
            ...
            dataList = chanDetail.ELEMENTS.memoList[idx2]
          } else {
          ...
          if (!payload[i].memoList || !payload[i].memoList.length === 0) {
            ...
            if (payload[i].jobkindId === 'BOAR') {
              ...
              chanList[idx1].ELEMENTS.memoList[idx2] = payload[i]
            } else {
      ...
      for (var i = 0; i < payload.length; i++) {
        ...
        if (idx2 !== -1) {
          if (payload[i].jobkindId === 'BOAR') {
            ...
            chanDetail.ELEMENTS.memoList[idx2].D_MEMO_LIST = payload[i].D_MEMO_LIST
          } else {
    ...
    MU_ADD_CONTENTS: (state, addContList) => {
      try {
        ...
        for (var i = 0; i < addContList.length; i++) {
          if (addContList[i] && addContList[i].jobkindId === 'TODO') addContList[i].creTeamKey = 0
          if (addContList[i] && addContList[i].jobkindId === 'MEMO') addContList[i].creTeamKey = -1
          ...
          } else if (addContList[i].jobkindId === 'MEMO') {
            idx2 = chanDetail.ELEMENTS.memoList.findIndex((item) => item.contentsKey === addContList[i].contentsKey)
          } else {
          ...
          if (idx2 === -1) {
            if (addContList[i].jobkindId === 'BOAR') {
              ...
              chanList[idx1].ELEMENTS.memoList.push(addContList[i])
              idx2 = chanDetail.ELEMENTS.memoList.findIndex((item) => item.contentsKey === addContList[i].contentsKey)
            } else {
          ...
          if (addContList[i].jobkindId === 'BOAR') {
            ...
            chanList[idx1].ELEMENTS.memoList[idx2] = addContList[i]
          } else if (addContList[i].jobkindId === 'TODO') {
    ...
    MU_ADD_MAIN_CHAN_LIST: (state, payload) => {
      if (!payload.initYn) {
        if (payload.ELEMENTS) {
          ...
          if ((!payload.ELEMENTS.memoList)) { payload.ELEMENTS.memoList = [] }
          ...
          payload.ELEMENTS = { alimList: [], boardList: [], todoList: [], memoList: [], cabinetList: [], commonList: { type: 'ALIM', list: [] }, managerList: [], showProfileUserList: [] }
        }
    ...
    MU_REPLACE_CONTENTS_ONLY_USERDO: (state, payload) => {
      ...
      for (var i = 0; i < payload.length; i++) {
        if (payload[i].jobkindId === 'BOAR') {
          ...
          idx2 = chanDetail.ELEMENTS.memoList.findIndex((item) => item.mccKey === payload[i].mccKey)
        } else {
        ...
        if (idx2 !== -1) {
          if (payload[i].jobkindId === 'BOAR') {
            ...
            chanDetail.ELEMENTS.memoList[idx2].D_CONT_USER_DO = payload[i].D_CONT_USER_DO
            chanDetail.ELEMENTS.memoList[idx2].D_CONT_USER_STICKER_LIST = payload[i].D_CONT_USER_STICKER_LIST
          } else {
  ...
  actions: {
    ...
    AC_ADD_CONTENTS: ({ commit, state }, payload) => { // 채널의 컨텐츠 부분 추가 (ALIM/BOARD)
      ...
      for (var i = 0; i < payload.length; i++) {
        ...
        payload[i].D_CONT_USER_DO = userDoList
        payload[i].D_CONT_USER_STICKER_LIST = payload[i].stickerList
    ...
    AC_SET_CHAN_CONT_LIST ({ state, commit }, payload) {
      ...
      for (var i = 0; i < payload.ELEMENTS.commonList.list.length; i++) {
        ...
        } else if (payload.ELEMENTS.commonList.type === 'MEMO') {
          dataList = payload.ELEMENTS.memoList
        } else {
